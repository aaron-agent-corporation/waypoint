...
All notable changes to `@projectrunner/spine` are documented here.

## 0.1.0 — 2026-05-06

Initial extraction from Mission Control (`feat/runner-runtime-slice` branch).
...
- **Command parser** — `/runner …` command grammar: `status`, `start plan`, `auto`, `auto status`, `routes`, `route`, `route-events`, `pause`, `resume`, `gate`, `discuss`, `doctor`, `forensics`, `help`.
- **Contracts** — host interfaces: `ISpineStore`, `ISpineAuthz`, `IEventBus`, `IRecipeRuntime`, plus `IClock` / `IIdGenerator`.
...
- **Routes** — `buildSpineRouteKey`, `normalizeSpineScope`, subject-type constants, autopilot progress helpers.
