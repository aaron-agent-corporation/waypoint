...
This example is a minimal project folder for trying the Waypoint folder host against the bundled `runner` Quest.
...
node ../../packages/waypoint-cli/src/bin.ts init --quest runner --postgres-no-durable
node ../../packages/waypoint-cli/src/bin.ts status
node ../../packages/waypoint-cli/src/bin.ts quests
node ../../packages/waypoint-cli/src/bin.ts recipes --quest runner
...
Run state (routes, tasks, events) lives in the project's postgres schema on the Console-managed instance (`deploy/postgres/`), not on disk. Task discussion JSONL stays in `.waypoint/tasks/`.
...
Run state can be cleared by dropping the project's schema from the postgres instance.
