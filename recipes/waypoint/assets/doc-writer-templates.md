...
> **Attribution:** These templates are ported verbatim from
> `agents/waypoint-doc-writer.md` in the get-shit-done-cc project
> (MIT, © 2025 Lex Christopherson). See
> `third_party/gsd/LICENSE` and `third_party/gsd/NOTICE.md`.

This file is the reference template bank for the `doc-writer` Recipe.
The Recipe prompt instructs the agent to load the matching
`<template_*>` section for the assigned doc type, rather than
embedding all templates into every prompt invocation.
...
1. NEVER include GSD methodology content in generated docs — no references to phases, plans, `/waypoint-` commands, PLAN.md, ROADMAP.md, or any GSD workflow concepts. Generated docs describe the TARGET PROJECT exclusively.
2. NEVER touch CHANGELOG.md — it is managed by `/waypoint-ship` and is out of scope.
3. Include the GSD marker `<!-- generated-by: waypoint-doc-writer -->` as the first line of every generated doc file (except supplement mode — see rule 7).
