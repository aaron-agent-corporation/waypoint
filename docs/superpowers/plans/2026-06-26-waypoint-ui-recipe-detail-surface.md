...
- **Backend-agnostic slug (G1).** The recipe slug is read through one shared `recipeSlugOf` helper that handles BOTH `metadata.waypoint.recipe.slug` (folder-host) AND `metadata.waypoint.recipe_slug` (Beads). Never read the slug path inline anywhere else.
- **Recipe-ness is the non-empty slug** — never a `node.type` field or the rendered badge string. `WaypointFolderTaskKind` is a 12-value union; "not a recipe" is the other eleven kinds, badged verbatim.
...
- **TDD.** Every task writes the failing test first. Run tests from `packages/waypoint-ui` with `npx vitest run <file>` (auto-uses the local `vitest.config.ts`).
...
- The spec aliased `Recipe = Pick<CatalogRecipeManifest, …>` re-exported from the engine package; `CatalogRecipeManifest` is not exported from `@waypoint/folder-host`'s index, so this plan defines `Recipe` as a standalone interface inside the UI.
...
**Files:**
- Create: `packages/waypoint-ui/src/recipe.ts`
- Test: `packages/waypoint-ui/src/recipe.test.ts`

**Interfaces:**
- Consumes: `WaypointFolderTask` from `./engine/types`.
- Produces:
  ...
  - `recipeSlugOf(task: Pick<WaypointFolderTask, 'metadata'>): string | null`
...
Create `packages/waypoint-ui/src/recipe.test.ts`:
...
describe('resolveRecipe', () => {
  ...
  it('returns undefined when present in neither loaded scope', () => {
    expect(resolveRecipe('missing', 'waypoint', caches)).toBeUndefined()
...
Run: `cd packages/waypoint-ui && npx vitest run src/recipe.test.ts`
...
```bash
git add packages/waypoint-ui/src/recipe.ts packages/waypoint-ui/src/recipe.test.ts
git commit -m "feat(ui): recipe slug/resolve helpers with dual-backend slug path (waypoint-8uf)"
```
...
Append to `packages/waypoint-ui/src/store.test.ts` (inside the existing `describe('store.applyMessage', …)` is fine, or add a new `describe`). Add a new block at the end of the file before the final `})` is closed — create a fresh describe:
...
git commit -m "feat(ui): store recipe caches + selection state, slug-aware selectTask/selectRoute (waypoint-8uf)"
```
...
**Files:**
- Create: `packages/waypoint-ui/src/components/RecipeCard.tsx`
- Test: `packages/waypoint-ui/src/components/RecipeCard.test.tsx`
...
Create `packages/waypoint-ui/src/components/RecipeCard.test.tsx`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/components/RecipeCard.test.tsx`
...
```bash
git add packages/waypoint-ui/src/components/RecipeCard.tsx packages/waypoint-ui/src/components/RecipeCard.test.tsx
git commit -m "feat(ui): RecipeCard component (name/desc/tools/prompt + fallbacks) (waypoint-8uf)"
```
...
**Files:**
- Modify: `packages/waypoint-ui/src/components/RoutesPanel.tsx`
...
Create `packages/waypoint-ui/src/components/RoutesPanel.test.tsx`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/components/RoutesPanel.test.tsx`
Expected: FAIL — the Recipes heading/rows/toggle do not exist yet.

- [ ] **Step 3: Write minimal implementation**

Replace the entire body of `packages/waypoint-ui/src/components/RoutesPanel.tsx`:
...
git commit -m "feat(ui): RoutesPanel Recipes section with Route|All scope toggle (waypoint-8uf)"
```
...
Create `packages/waypoint-ui/src/components/TaskDetail.test.tsx`:
...
Replace the entire body of `packages/waypoint-ui/src/components/TaskDetail.tsx`:
...
git commit -m "feat(ui): TaskDetail dispatches to RecipeCard on recipe selection (waypoint-8uf)"
```
...
**Files:**
- Modify: `packages/waypoint-ui/src/graph/build-graph.ts`
- Create: `packages/waypoint-ui/src/components/GraphNode.tsx`
- Modify: `packages/waypoint-ui/src/components/RouteGraph.tsx`
- Test: `packages/waypoint-ui/src/graph/build-graph.test.ts` (extend if present; create if absent)
- Test: `packages/waypoint-ui/src/components/GraphNode.test.tsx`
...
Create `packages/waypoint-ui/src/graph/build-graph.test.ts` (if a build-graph test already exists, append these cases instead):
...
Create `packages/waypoint-ui/src/components/GraphNode.test.tsx`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/graph/build-graph.test.ts src/components/GraphNode.test.tsx`
Expected: FAIL — `data.badge` is undefined; `./GraphNode` does not resolve.

- [ ] **Step 3a: Enrich `build-graph.ts`**

In `packages/waypoint-ui/src/graph/build-graph.ts`, add the import at the top:
...
Replace the body of `packages/waypoint-ui/src/components/RouteGraph.tsx`:
...
```bash
git add packages/waypoint-ui/src/graph/build-graph.ts packages/waypoint-ui/src/components/GraphNode.tsx packages/waypoint-ui/src/components/GraphNode.test.tsx packages/waypoint-ui/src/components/RouteGraph.tsx packages/waypoint-ui/src/graph/build-graph.test.ts
git commit -m "feat(ui): graph node badges + recipe-name subtitle via injected resolver (waypoint-8uf)"
```
...
Append to `packages/waypoint-ui/src/App.test.tsx` a new test inside the existing `describe('App', …)` block:
...
Run: `cd packages/waypoint-ui && npx vitest run src/App.test.tsx`
Expected: FAIL — no `catalog.recipes` call is ever made (`recipesByQuest['code-review']` stays undefined).

- [ ] **Step 3: Write minimal implementation**

In `packages/waypoint-ui/src/App.tsx`:
...
```bash
git add packages/waypoint-ui/src/App.tsx packages/waypoint-ui/src/App.test.tsx
git commit -m "feat(ui): fetch catalog.recipes per scope with cache + recipesError banner (waypoint-8uf)"
```
...
Run: `cd packages/waypoint-ui && npx tsc --noEmit -p tsconfig.json`
Expected: no errors. (If the package has no standalone `tsconfig.json`, run the repo's typecheck script: `cd ~/Agent-Corporation/waypoint && npm run typecheck`.)
...
With an engine host running (handshake env set), `WAYPOINT_ENGINE_HANDSHAKE=… pnpm dev:ui`, open `http://localhost:5273`, select a route, confirm the Recipes rail lists the quest's recipes, click a recipe node and a rail recipe, and toggle **All**. Verify a recipe card shows name/description/tools/prompt.

- [ ] **Step 4: Commit (only if Step 1–2 required any fix)**

```bash
git add -A packages/waypoint-ui
git commit -m "chore(ui): typecheck + full-suite green for recipe-detail surface (waypoint-8uf)"
```
...
- **Do not touch any package other than `packages/waypoint-ui`.** If you find yourself editing `@waypoint/folder-host` or `@waypoint/engine-host`, stop — the slug helper and `Recipe` type live entirely in the UI by design (Global Constraints).
- **The slug path lives in exactly one place** (`recipeSlugOf`). If you need the slug anywhere, call the helper; never re-read `metadata.waypoint.*` inline.
