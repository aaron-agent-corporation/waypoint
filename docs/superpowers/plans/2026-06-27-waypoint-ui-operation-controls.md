...
- **One engine change only:** `catalog.install` in `packages/waypoint-engine-host`. Everything else is `packages/waypoint-ui`. No proxy/token/permission change (the UI proxy already carries the unrestricted `global` token and forwards all `/cmd/*`).
...
- **`gate.decide` node** comes from `route.current_node`, falling back to the gate task identifier (`plan_ref` → `id` → `beads_id`) when null — NOT `metadata.waypoint.node_key` (the engine matcher reads `plan_ref`/`id`/`beads_id`, `beads/transitions.ts`).
- **`discuss.post` omits `author`** — the engine applies its default identity.
- **`catalog.install` is workspace-wins:** an authored `.waypoint/quests/<slug>.yaml` is preserved, only missing recipes installed; returns the full envelope `{ quest, recipes, installedQuestPaths, installedRecipePaths }`. Real `installQuestCatalog` signature is 3-arg `(projectRoot, catalog, options)`.
- **No optimistic UI:** no control mutates local state before the engine confirms; the UI reflects engine state and refetches.
- **TDD.** Run UI tests from `packages/waypoint-ui` with `npx vitest run <file>`; engine tests from repo root with `npx vitest run <file>`; `npm run typecheck` from repo root.
...
- Test: `packages/waypoint-engine-host/src/core/commands/catalog.test.ts` (create if absent; else extend)

**Interfaces:**
- Consumes: `loadBundledWaypointCatalog`, `installQuestCatalog` from `@waypoint/folder-host`; `EngineError`, `ok` from `../../envelope.ts`.
...
Add to `packages/waypoint-engine-host/src/core/commands/catalog.test.ts` (mirror the existing `catalog.quests`/`catalog.recipes` test harness in that file — same `registerCatalogCommands` + a temp workspace with an active session). If the file does not exist, model it on `packages/waypoint-engine-host/src/__tests__/workspace-catalog.test.ts` (which already dispatches `catalog.*` against a real workspace). The new cases:
...
it('catalog.install installs a bundled quest and returns the full envelope', async () => {
  ...
  const installed = await readFile(join(root, '.waypoint/quests/add-tests.yaml'), 'utf8')
...
it('catalog.install is workspace-wins: an authored quest manifest is byte-identical after re-install', async () => {
  ...
  const path = join(root, '.waypoint/quests/add-tests.yaml')
...
Run: `npx vitest run packages/waypoint-engine-host/src/core/commands/catalog.test.ts`
Expected: FAIL — `Unknown command: catalog.install` (or the dispatch returns a not-registered failure).

- [ ] **Step 3: Implement the command**

In `packages/waypoint-engine-host/src/core/commands/catalog.ts`:
...
3c. `installQuestCatalog` must honor `preserveExistingQuest`. Open `packages/waypoint-folder-host/src/catalog/install.ts`. Add the option to `InstallQuestCatalogOptions`:
...
Run: `npx vitest run packages/waypoint-folder-host/src/catalog/install.test.ts`
Expected: PASS.

- [ ] **Step 5: Commit**

```bash
git add packages/waypoint-engine-host/src/core/commands/catalog.ts packages/waypoint-engine-host/src/core/commands/catalog.test.ts packages/waypoint-folder-host/src/catalog/install.ts
git commit -m "feat(engine): catalog.install command (workspace-wins) (waypoint-aws)"
```
...
**Files:**
- Create: `packages/waypoint-ui/src/lib/engine.ts`
- Modify: `packages/waypoint-ui/src/App.tsx` (remove the local `listField`, import it)
- Test: `packages/waypoint-ui/src/lib/engine.test.ts`
...
Create `packages/waypoint-ui/src/lib/engine.test.ts`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/lib/engine.test.ts`
...
In `packages/waypoint-ui/src/App.tsx`: delete the local `listField` function definition (the `function listField<K extends string, T>(...) {...}` block) and add the import near the other local imports:
...
Run: `cd packages/waypoint-ui && npx vitest run src/lib/engine.test.ts src/App.test.tsx`
Expected: PASS — the new helper tests plus all existing App tests (App still unwraps envelopes identically).

- [ ] **Step 5: Commit**

```bash
git add packages/waypoint-ui/src/lib/engine.ts packages/waypoint-ui/src/lib/engine.test.ts packages/waypoint-ui/src/App.tsx
git commit -m "refactor(ui): extract shared listField into lib/engine (waypoint-aws)"
```
...
**Files:**
- Modify: `packages/waypoint-ui/src/store.ts`
- Modify: `packages/waypoint-ui/src/App.tsx`
...
Append to the `store recipe state` describe (or a new describe) in `packages/waypoint-ui/src/store.test.ts`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/store.test.ts`
Expected: FAIL — `setControlError is not a function`.

- [ ] **Step 3: Implement**

In `packages/waypoint-ui/src/store.ts`:
...
3e. In `packages/waypoint-ui/src/App.tsx`, add the selector inside `Console` (after `recipesError`):
...
Run: `cd packages/waypoint-ui && npx vitest run src/store.test.ts src/App.test.tsx`
Expected: PASS.

- [ ] **Step 5: Commit**

```bash
git add packages/waypoint-ui/src/store.ts packages/waypoint-ui/src/store.test.ts packages/waypoint-ui/src/App.tsx
git commit -m "feat(ui): controlError store field + banner row (waypoint-aws)"
```
...
**Files:**
- Create: `packages/waypoint-ui/src/engine/useEngineCommand.ts`
- Test: `packages/waypoint-ui/src/engine/useEngineCommand.test.tsx`
...
Create `packages/waypoint-ui/src/engine/useEngineCommand.test.tsx`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/engine/useEngineCommand.test.tsx`
...
```bash
git add packages/waypoint-ui/src/engine/useEngineCommand.ts packages/waypoint-ui/src/engine/useEngineCommand.test.tsx
git commit -m "feat(ui): useEngineCommand hook (in-flight + controlError + routesEpoch) (waypoint-aws)"
```
...
**Files:**
- Create: `packages/waypoint-ui/src/components/ConfirmAction.tsx`
- Test: `packages/waypoint-ui/src/components/ConfirmAction.test.tsx`
...
Create `packages/waypoint-ui/src/components/ConfirmAction.test.tsx`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/components/ConfirmAction.test.tsx`
...
```bash
git add packages/waypoint-ui/src/components/ConfirmAction.tsx packages/waypoint-ui/src/components/ConfirmAction.test.tsx
git commit -m "feat(ui): ConfirmAction inline-confirm component (waypoint-aws)"
```
...
**Files:**
- Create: `packages/waypoint-ui/src/components/StartQuest.tsx`
- Modify: `packages/waypoint-ui/src/components/RoutesPanel.tsx` (mount `<StartQuest />` in the Routes header)
- Test: `packages/waypoint-ui/src/components/StartQuest.test.tsx`
...
Create `packages/waypoint-ui/src/components/StartQuest.test.tsx`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/components/StartQuest.test.tsx`
...
In `packages/waypoint-ui/src/components/RoutesPanel.tsx`, import and mount it in the Routes header:
...
Run: `cd packages/waypoint-ui && npx vitest run src/components/StartQuest.test.tsx src/components/RoutesPanel.test.tsx`
Expected: PASS — StartQuest tests plus the existing RoutesPanel tests still green.

- [ ] **Step 5: Commit**

```bash
git add packages/waypoint-ui/src/components/StartQuest.tsx packages/waypoint-ui/src/components/StartQuest.test.tsx packages/waypoint-ui/src/components/RoutesPanel.tsx
git commit -m "feat(ui): StartQuest picker — install→start, confirmed (waypoint-aws)"
```
...
**Files:**
- Create: `packages/waypoint-ui/src/routeStatus.ts` (pure paused-provenance helper)
- Modify: `packages/waypoint-ui/src/engine/types.ts` (re-export route-event types)
- Modify: `packages/waypoint-ui/src/store.ts` (route-events cache)
- Modify: `packages/waypoint-ui/src/App.tsx` (fetch route events for blocked routes)
- Modify: `packages/waypoint-ui/src/components/RoutesPanel.tsx` (Pause/Resume buttons)
- Test: `packages/waypoint-ui/src/routeStatus.test.ts`, and Pause/Resume cases in `RoutesPanel.test.tsx`

**Interfaces:**
- Produces: `routeIsPaused(events: readonly WaypointFolderRouteEvent[]): boolean` — true iff the latest `route.paused`/`route.resumed` event is `route.paused`. Store `routeEventsByRoute: Record<string, WaypointFolderRouteEvent[]>` + `setRouteEvents(routeId, events)`.

- [ ] **Step 1: Write the failing tests**

Create `packages/waypoint-ui/src/routeStatus.test.ts`:
...
Add Pause/Resume cases to `packages/waypoint-ui/src/components/RoutesPanel.test.tsx` (inside the `RoutesPanel routes/sessions` describe or a new one):
...
(Ensure the test file imports `ClientProvider` from `../engine/context`, `fireEvent`/`waitFor` from `@testing-library/react`, and `FakeEngineClient` from `../test/fake-client`; add a `route(id, quest)` helper that returns a `WaypointFolderRoute` with `status: 'active'` if one is not already present.)

- [ ] **Step 2: Run tests to verify they fail**

Run: `cd packages/waypoint-ui && npx vitest run src/routeStatus.test.ts src/components/RoutesPanel.test.tsx`
Expected: FAIL — `./routeStatus` unresolved; no Pause/Resume buttons.

- [ ] **Step 3a: Pure helper + type re-exports**

Create `packages/waypoint-ui/src/routeStatus.ts`:
...
In `packages/waypoint-ui/src/engine/types.ts`, add `WaypointFolderRouteEvent` and `RouteEventPage` to the `import type {…} from '@waypoint/folder-host'` block and the `export type {…}` re-export block.

- [ ] **Step 3b: Store route-events cache**

In `packages/waypoint-ui/src/store.ts`: add `WaypointFolderRouteEvent` to the type imports; add state `routeEventsByRoute: Record<string, WaypointFolderRouteEvent[]>` (init `{}`), action signature `setRouteEvents(routeId: string, events: WaypointFolderRouteEvent[]): void`, and the implementation:
...
Add `WaypointFolderRouteEvent` to the `engine/types` import in `App.tsx`. (`route.events` returns `{ events: RouteEventPage }` where `RouteEventPage` is `{ items, total }`.)
...
Run: `cd packages/waypoint-ui && npx vitest run`
Expected: PASS.

- [ ] **Step 5: Commit**

```bash
git add packages/waypoint-ui/src/routeStatus.ts packages/waypoint-ui/src/routeStatus.test.ts packages/waypoint-ui/src/engine/types.ts packages/waypoint-ui/src/store.ts packages/waypoint-ui/src/App.tsx packages/waypoint-ui/src/components/RoutesPanel.tsx packages/waypoint-ui/src/components/RoutesPanel.test.tsx
git commit -m "feat(ui): Pause/Resume via paused-provenance from route events (waypoint-aws)"
```
...
**Files:**
- Modify: `packages/waypoint-ui/src/components/TaskDetail.tsx`
...
Add to the `TaskDetail dispatcher` describe in `packages/waypoint-ui/src/components/TaskDetail.test.tsx`:
...
(The test file already imports `WaypointFolderTask`, `route` fixture, `useStore`, `render`, `screen`; add `ClientProvider` from `../engine/context`, `fireEvent`/`waitFor` from `@testing-library/react`, and `FakeEngineClient` from `../test/fake-client`. Note these tests render TaskDetail inside `ClientProvider` — wrap existing renders too if a hook now requires the client.)

- [ ] **Step 2: Run tests to verify they fail**

Run: `cd packages/waypoint-ui && npx vitest run src/components/TaskDetail.test.tsx`
Expected: FAIL — no gate controls.

- [ ] **Step 3: Implement**

In `packages/waypoint-ui/src/components/TaskDetail.tsx`: import the hook + ConfirmAction; in the non-recipe branch, when `task.kind === 'gate'`, render the controls. Add near the top of the component body:
...
(`beads_id` is not on the `WaypointFolderTask` UI type; `plan_ref`/`id` cover the fallback the engine matcher accepts. If `current_node` is set, that wins, matching the spec.)
...
git commit -m "feat(ui): gate Approve/Reject in TaskDetail (waypoint-aws)"
```
...
**Files:**
- Create: `packages/waypoint-ui/src/components/DiscussionThread.tsx`
- Modify: `packages/waypoint-ui/src/engine/types.ts` (re-export discussion types)
- Modify: `packages/waypoint-ui/src/components/TaskDetail.tsx` (render thread for `discussion` tasks)
- Test: `packages/waypoint-ui/src/components/DiscussionThread.test.tsx`
...
Create `packages/waypoint-ui/src/components/DiscussionThread.test.tsx`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/components/DiscussionThread.test.tsx`
Expected: FAIL — `Failed to resolve import './DiscussionThread'`.

- [ ] **Step 3: Implement**

In `packages/waypoint-ui/src/engine/types.ts`, add `TaskDiscussionMessagePage` and `WaypointTaskDiscussionMessage` to the folder-host import + re-export blocks.
...
In `packages/waypoint-ui/src/components/TaskDetail.tsx`, in the task-fields branch, render the thread when `task.kind === 'discussion'`:
...
Run: `cd packages/waypoint-ui && npx vitest run src/components/DiscussionThread.test.tsx src/components/TaskDetail.test.tsx`
Expected: PASS.

- [ ] **Step 5: Commit**

```bash
git add packages/waypoint-ui/src/components/DiscussionThread.tsx packages/waypoint-ui/src/components/DiscussionThread.test.tsx packages/waypoint-ui/src/engine/types.ts packages/waypoint-ui/src/components/TaskDetail.tsx
git commit -m "feat(ui): discussion thread (list + post) in TaskDetail (waypoint-aws)"
```
...
**Files:**
- Modify: `packages/waypoint-ui/src/components/AgentChat.tsx`
- Test: `packages/waypoint-ui/src/components/AgentChat.test.tsx` (create if absent)
...
Create/extend `packages/waypoint-ui/src/components/AgentChat.test.tsx`. First read `AgentChat.tsx` to mirror how it selects the active session + transcript from the store (it reads `activeSessionId` and `transcripts[sessionId]`). Model the test on that:
...
Run: `cd packages/waypoint-ui && npx vitest run src/components/AgentChat.test.tsx`
Expected: FAIL — no Approve proposal button.

- [ ] **Step 3: Implement**

In `packages/waypoint-ui/src/components/AgentChat.tsx`: add a helper that extracts a `proposalId` from a tool-result record's text, and render the confirm control when present. Import:
...
git commit -m "feat(ui): approve proposal from AgentChat transcript (waypoint-aws)"
```
...
Run: `cd /Users/aaronwhaley/Agent-Corporation/waypoint && npm run typecheck`
...
Run: `npx vitest run packages/waypoint-engine-host/src/core/commands/catalog.test.ts packages/waypoint-folder-host/src/catalog/install.test.ts`
Expected: PASS.
...
With an engine host running (handshake env set), `WAYPOINT_ENGINE_HANDSHAKE=… pnpm dev:ui`, open the console: Start a quest (confirm), Pause then Resume the new route, select a gate node and Approve, select a discussion node and Post, and (if an agent run produced one) Approve a proposal in the chat pane.

- [ ] **Step 5: Commit (only if Steps 1–3 required a fix)**

```bash
git add -A packages/waypoint-ui packages/waypoint-engine-host packages/waypoint-folder-host
git commit -m "chore: typecheck + full-suite green for operation controls (waypoint-aws)"
```
...
- **Do not touch any package other than the three named** (`waypoint-ui`, plus the single `catalog.install` change spanning `waypoint-engine-host` + `waypoint-folder-host/src/catalog/install.ts`).
