<!-- /autoplan restore point: /Users/aaronwhaley/.gstack/projects/aaron-agent-corporation-runner/main-autoplan-restore-20260618-222531.md -->
# Spine Pi Agent Brain (Slice 2) Implementation Plan
...
- **Pi version is pinned.** A single `PI_PINNED_RANGE` constant gates compatibility; `SPINE_BRAIN=pi` with a missing/wrong-version `pi` **fails loud** (never silent-falls-back to the fake). Active brain (`pi`|`fake` + version) is reported in `meta.health` / `meta.version`.
...
New code under `packages/spine-engine-host/src/`:
...
`packages/spine-folder-host/src/` changes (ad-hoc execution seam — pinned by Task 0 Spike 2):
```
routes/start-adhoc.ts     # overlay-resolving route start (no writes to .runner/quests|recipes)
autopilot/run.ts          # runSpineAutopilot accepts { signal?, catalogDir? } (overlay + abort)
runtime/local-runtime.ts  # LocalRecipeRuntime.runRecipe accepts an AbortSignal; SIGTERM→SIGKILL child group
```

New sibling package: `packages/spine-pi-extension/` (entry + tools + host-client + README).
...
8. **Route-event seq is grounded in the durable log.** EventHub route sequencing derives from `readSpineRuntimeRouteEvents` (the durable event log is the source of truth); command-returned deltas are only low-latency *hints* that trigger an immediate broadcaster read (no polling delay). (Consistent with slice-1 `RouteBroadcaster`; Task 1/7.)
...
**Files:** Create `docs/superpowers/spikes/2026-06-18-pi-agent-brain-spikes.md`; capture fixtures under `packages/spine-engine-host/src/brain/__fixtures__/pi-stream/`.

**Produces:** findings pinning (1) Pi extension/tool-registration API + the exact JSON event lines + the working `pi` version; (2) the **session-overlay** ad-hoc execution seam — how `runSpineAutopilot` can resolve recipes from a session-scoped dir and whether an `AbortSignal` can thread through to `LocalRecipeRuntime`.

- [ ] **Step 1: Spike 1 — Pi extension API + version + fixtures.** Against installed `pi`, confirm: an extension can register a named tool with a JSON-schema input; the handler receives parsed args; it can read env (`SPINE_HOST_URL`/`SPINE_HOST_TOKEN`); it returns a result the agent sees. Record `pi --version` (this becomes `PI_PINNED_RANGE`). Capture verbatim JSON event lines (session/message/toolcall/tool_execution/turn_end/agent_end) into `__fixtures__/pi-stream/basic-session.jsonl`, plus a **split-chunk** sample (a record broken across two reads) and a **multi-record-in-one-chunk** sample. If custom-tool registration is not cleanly supported: **do not silently fall back to bash+CLI** (it widens blast radius for an unrestricted agent — Eng/CEO finding). Stop and surface; the bash fallback requires its own explicit decision.
- [ ] **Step 2: Spike 2 — session-overlay execution seam.** Verified facts: `startQuestRoute` resolves recipes via `loadBundledSpineCatalog().resolveQuestRecipes` (`routes/start.ts:45`) and reads `.runner/quests/<slug>.yaml` (`:60,:135`); `runSpineAutopilot` reads recipes from disk (`autopilot/run.ts`); `LocalRecipeRuntime.runRecipe` has **no** `AbortSignal` today. Prototype in `/tmp`: write a draft quest+recipe under a **session dir** `.runner/agent/<sid>/catalog/{quests,recipes}/`, then start + autopilot a route that resolves from that dir (NOT the bundle, NOT the live catalog). Confirm the minimal change to (a) point recipe resolution at the overlay dir, (b) thread an `AbortSignal` through `runSpineAutopilot` → `LocalRecipeRuntime` so cancel kills the in-flight child. Record the exact functions/signatures to change. If abort threading needs deep runtime surgery, scope it explicitly.
- [ ] **Step 3: Write findings + commit fixtures.**

```bash
git add docs/superpowers/spikes/2026-06-18-pi-agent-brain-spikes.md \
        packages/spine-engine-host/src/brain/__fixtures__/pi-stream/
...
**Produces:** `AgentEventRecord = { id, sessionId, kind, at, data? }`; `EngineEvent.record: SpineFolderRouteEvent | AgentEventRecord`; `EventHub.publishAgent(topic, record)`. **Agent events get a separate ring budget** so a chatty agent run can't evict `route:*` events a reconnecting WS subscriber needs (Eng decision 15).
...
const root = async () => { const r = await mkdtemp(join(tmpdir(), 'wp-as-')); await mkdir(join(r, '.runner'), { recursive: true }); return r }
...
describe('AgentSession', () => {
  it('republishes on agent:<id>, writes transcript, and fires onTerminal', async () => {
    ...
    expect(await readFile(join(r, '.runner', 'agent', 's1.jsonl'), 'utf8')).toContain('agent.message')
...
**Files:** Create `packages/spine-folder-host/src/routes/start-adhoc.ts`; Modify `packages/spine-folder-host/src/autopilot/run.ts`, `packages/spine-folder-host/src/runtime/local-runtime.ts`, `packages/spine-folder-host/src/index.ts`, `packages/spine-engine-host/src/core/commands/run.ts`; Tests `start-adhoc.test.ts`, `run.adhoc.test.ts`, plus an autopilot-abort test.
...
**Produces:**
- `startAdhocRoute(projectRoot, opts: { sessionId, questYaml, recipeYamls, dryRun?, signal?, now? }): Promise<StartedQuestRoute>` — writes drafts to the **session overlay** `.runner/agent/<sessionId>/catalog/{quests,recipes}/` (never `.runner/quests|recipes`); materializes the route + tasks resolving recipes from the overlay; metadata `{ adhoc: true, sessionId, overlay: '<dir>' }`. If `!dryRun`, runs `runSpineAutopilot(projectRoot, { routeId, catalogDir: overlayDir, signal })`.
- `runSpineAutopilot` gains `{ signal?: AbortSignal; catalogDir?: string }`; passes `signal` to the runtime and resolves recipes from `catalogDir` when given.
...
- [ ] **Step 1: Failing folder-host test** — `startAdhocRoute` starts a route from a workspace-absent draft, writes NOTHING under `.runner/quests|recipes`, materializes tasks, and (dryRun:false) executes:
...
it('starts + executes from a session overlay without touching the live catalog', async () => {
  ...
  expect(existsSync(join(root, '.runner', 'quests', 'adhoc-demo.yaml'))).toBe(false)   // no catalog write
  expect(existsSync(join(root, '.runner', 'agent', 's1', 'catalog', 'quests', 'adhoc-demo.yaml'))).toBe(true)
  const tasks = await listSpineRuntimeTasks(root, { routeId: r.id })
...
- `PiCliBrainAdapter` — spawns `pi -p --mode json --tools <grant> -e <ext> <intent>` with env `SPINE_HOST_URL`/`SPINE_HOST_TOKEN` (the **scoped** token) + system prompt; pipes stdout through `PiStreamDecoder`; captures stderr (tail in error); on abort SIGTERM then **SIGKILL after a timeout**; on close maps exit/condition → `BrainResult`.
...
## Task 8: Spine Pi extension package

**Files:** `packages/spine-pi-extension/` (`package.json`, `src/host-client.ts`, `src/tools.ts`, `src/index.ts`, `src/README.md`, tests); Modify `tsconfig.json`, `vitest.config.ts`, `scripts/stage-package-builds.mjs`.

**Produces:** `createHostClient({ url, token, fetchImpl? })` (loopback `POST /cmd/<name>` with bearer; maps 401/403 to actionable errors — Task 10/decision 19); `buildSpineTools(client)` → granted tools only (snake_case command names; excludes `approveProposal`); `src/index.ts` registers them with Pi per Spike 1; a package README (install, the two env vars it reads, the tool list).
...
- [ ] **Step 9: Commit** `feat(pi-extension): Spine tools over the engine-host HTTP API (scoped token)`
...
**Produces:**
- `bin.ts`/`engine-host.ts`: when `SPINE_BRAIN=pi`, require `piVersionOk()`; if `pi` missing or version-mismatched → **fail loud** (throw a clear `problem+cause+fix` error), never silently use the fake. Otherwise default to `FakeBrainAdapter`. Adapter set via the holder after `start()` yields `{ url, token }`.
...
**Files:** Modify `src/envelope.ts`/`src/types.ts` (confirm `details: { code, path, message }`, `field`→`path` mapping), `src/transport/http-ws/server.ts` (status mapping), `packages/spine-pi-extension/src/host-client.ts`; align any tests reading `details`; Test `src/envelope.shape.test.ts`.
...
- [ ] **Step 2: Run → FAIL.**
- [ ] **Step 3: Implement** the normalization; host-client maps HTTP 401→"authorization failed: SPINE_HOST_TOKEN missing/invalid", 403→"tool not in session grant; allowed: [...]" (read allowed list from the FORBIDDEN envelope).
...
**Files:** Create `scripts/agent-brain-smoke.mjs`; Modify root `package.json` (`smoke:agent-brain`), `README.md`, ensure `packages/spine-pi-extension/README.md`.
...
- [ ] **Step 4** README agent section rewritten to the **adopted** surface:
  ...
  - **Environment** table: `SPINE_BRAIN`, `SPINE_HOST_URL`, `SPINE_HOST_TOKEN`, `SPINE_PI_EXTENSION` (var / who-sets / default / required-when), noting URL+TOKEN are injected by the host into the Pi child.
  ...
  - disk footprint: `.runner/agent/<id>.jsonl` + `.runner/agent/<sessionId>/catalog/`.
...
**VERDICT:** Approved with decisions 2–22 integrated (premise held by user: unrestricted-by-default, now watchable + host-enforced scoping). Test-plan artifact: `~/.gstack/projects/aaron-agent-corporation-runner/aaron-main-test-plan-*.md`. Next: `/mar`.
