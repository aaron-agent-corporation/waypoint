<!--
  Canonical implementation plan for the Spine Engine Host (slice 1).
...
# Spine Engine Host + Local API — Implementation Plan (slice 1)
...
**Goal:** Build `@projectrunner/spine-engine-host`, a resident Node process wrapping `@projectrunner/spine` + `@projectrunner/spine-folder-host`, exposing the full run + watch + author contract over a transport-agnostic command/event core with a loopback HTTP + WebSocket adapter.

**Architecture:** A pure command/event core — `CommandBus` (named command → envelope) and `EventHub` (seq'd live event fan-out) — sits over a `WorkspaceSession` that binds one Spine workspace and delegates every operation to existing backend-agnostic folder-host functions. A thin HTTP+WS adapter translates wire ↔ core; `transport.ts` is the seam where Tauri IPC later drops in.

**Live-events design (corrected — see Task 7):** folder-host's `IEventBus` is publish-only, so there is no live subscription to tap. **The durable route-event log (`readSpineRuntimeRouteEvents`) is the single source of truth** for what gets broadcast; live deltas are produced by **event-ID set-diffing that durable log** (publish only events whose stable event ID has not been broadcast before), serialized per route. A mutating command's returned delta is used **only as a low-latency hint** that triggers an immediate broadcaster read of the durable log — never as the broadcast payload itself — which eliminates polling delay without inventing a second event source (resolved: response-gemini-1-issue-1). Integer-offset paging is **rejected** — Beads re-sorts events with positional IDs, so offsets drop/dupe. A periodic poll tick remains as the fallback that surfaces out-of-band writes (autopilot / Crew / external) for which no command hint fires.

**Tech Stack:** TypeScript (ESM, `type: module`), Node 22, `@projectrunner/spine` (`workspace:*`), `@projectrunner/spine-folder-host` (`workspace:*`), `ws`, Vitest. Beads path uses the `bd`-spawning client folder-host already constructs.
...
- `makeErrorEnvelope(error, details?) → { ok:false, action:'error', error, details? }` — `@projectrunner/spine`.
- `initSpineProject(root, { quest, backend?, now? })` — folder-host (does **not** install the bundled catalog — see Task 6/Correction A).
- `readSpineStatus(root, options?) → { initialized, quest, backend, routes, beads, ... }`.
- `loadBundledSpineCatalog() → { quests, recipes, resolveQuestRecipes(quest) }` — `quests`/`recipes` are **registries**; use `.list()` (Correction B).
- `startQuestRoute(root, { quest }) → StartedQuestRoute` — reads `.runner/quests/<quest>.yaml`, so the catalog must be installed first.
- `listSpineRuntimeRoutes/getSpineRuntimeRoute/listSpineRuntimeTasks`, `readSpineRuntimeRouteEvents(root, routeId, { limit?, offset? }) → { items: SpineFolderRouteEvent[]; total; limit; offset }`, `SpineFolderRouteEvent = { id, route_id, kind, created_at, payload? }`.
- `approveRouteGate/rejectRouteGate(root, { routeId, node, note?, nextNode?, now? })`, `pauseSpineRoute/resumeSpineRoute`.
...
- **Client-injection seam (Task-5 gate, resolved: response-claude-1-issue-1):** confirm whether `startQuestRoute`/`approveRouteGate`/`rejectRouteGate`/`pauseSpineRoute`/`resumeSpineRoute`/`appendTaskDiscussionMessage` and the read APIs accept an options bag exposing an `issueClient`/`beadsClient`. `> confirm against spine-folder-host/src/**; if absent, add the injection parameter in Task 5.`
...
### Task 1: Scaffold `@projectrunner/spine-engine-host`
...
describe('engine-host package', () => {
  it('reports its package identity', () => {
    expect(getEngineHostInfo()).toEqual({ packageName: '@projectrunner/spine-engine-host', corePackage: 'runner-core' })
  })
})
```
- [ ] **Manifest** (`name:"@projectrunner/spine-engine-host"`, `type:"module"`, `bin:{ "spine-engine-host":"./dist/bin.js" }`, deps: `@projectrunner/spine`/`@projectrunner/spine-folder-host` `workspace:*`, `ws ^8.18`, dev `@types/ws`).
- [ ] **Entry export** `getEngineHostInfo(): { packageName, corePackage }`.
- [ ] **Path mapping** in `tsconfig.json`: `"@projectrunner/spine-engine-host": ["./packages/spine-engine-host/src/index.ts"]`.
- [ ] **Deps:** `pnpm add ws --filter @projectrunner/spine-engine-host && pnpm add -D @types/ws --filter @projectrunner/spine-engine-host`.
- [ ] **Verify** `pnpm vitest run packages/spine-engine-host && pnpm typecheck`; commit `feat(engine-host): scaffold package`.
...
- [ ] **`types.ts`:**
```ts
import type { SpineErrorEnvelope } from '@projectrunner/spine'
import type { SpineFolderRouteEvent } from '@projectrunner/spine-folder-host'
...
export type EngineEnvelope = EngineSuccessEnvelope | SpineErrorEnvelope
export interface EngineEvent { readonly seq: number; readonly topic: string; readonly record: SpineFolderRouteEvent }
```
> If `SpineFolderRouteEvent` is not re-exported, add `export type { SpineFolderRouteEvent } from './events/types.ts'` to folder-host's `index.ts` and commit it here.
...
**Produces:** `open(input)` that, on a fresh root, runs the **same sequence as `spine-cli/src/commands/init.ts`**: optional Beads init (when `initBeads` or `backend:'beads'`) → `initSpineProject` → **install bundled catalog** (`installQuestCatalog` or the init helper — `> confirm the exact export against init.ts`) → readiness check. For an existing workspace, derive backend from `readSpineStatus` and **reject a conflicting `backend` argument** (`CONFLICT`). Plus:
...
- [ ] **`commands/catalog.ts`** returns `.list()` arrays; for `resolveQuestRecipes`, `if (resolved.ok === false) throw new EngineError(resolved.message, { code:'NOT_FOUND', field:'quest' })`. `> confirm registry/`.list()`/resolve result shape against spine-folder-host/src/catalog/bundled.ts.`
- [ ] **`commands/meta.ts`** registers the three meta commands; `meta.health` reads `session.current() != null` and `hub.currentSeq()`.
- [ ] **EngineHost shell** wires `session`, `hub`, `bus`, a `RouteBroadcaster` (Task 7), registers workspace/catalog/meta now and run/gate/author in later tasks; exposes `dispatch`, `snapshot()` (`Promise.all([listSpineRuntimeRoutes, listSpineRuntimeTasks])`), `start/stop` (Task 10).
...
- `emit(routeId)`: under a per-route async mutex, **read the durable log** `readSpineRuntimeRouteEvents(root, routeId, { limit: <large> })`, publish every item whose `id` is not yet in the set, then add it. **Idempotent under interleaving; correct for both backends** (no positional-offset assumption).
...
- **Out-of-band writes:** a poll tick (~3s) runs the same ID-diff `emit` for every route that currently has subscribers, surfacing autopilot / Crew / external events for which **no command hint fired**. `> open decision: ship the tick in slice 1 or defer — default: ship, gated behind "has subscribers" so idle workspaces don't poll.`
...
- [ ] **`discuss.post`** uses the real arity: `appendTaskDiscussionMessage(root, input.taskId, { content: input.message, author: input.author ?? 'user' })`, run **inside `session.mutate`** (layer (a)). Add **`discuss.list`** wrapping `readTaskDiscussionMessages` (read counterpart). `> confirm field names against spine-folder-host/src/discussion/store.ts.`
...
- [ ] **`bin.ts` — `startEngineHostFromEnv(argv, env)`:** boot host, open `SPINE_ENGINE_ROOT` (backend from `SPINE_ENGINE_BACKEND`), write an **atomic handshake**: write `*.tmp`, `chmod 0600`, `rename`; record `{ schemaVersion, pid, url, token, createdAt, workspaceRoot }`; delete a stale file at startup; unlink on clean shutdown. Direct-run stdout prints the **full record (incl. token) or nothing** — never a token-less line that disagrees with the file.
...
- [ ] **Real-bd gate:** reuse the `realBdAvailable()` pattern from `spine-cli/src/commands/beads-backend-smoke.test.ts`. The Beads lane **skips locally without bd** but is **required in CI** (bd + Dolt installed). `> confirm realBdAvailable() export/location.`
...
- [ ] Verify (`pnpm vitest run packages/spine-engine-host && pnpm typecheck`) + commit `test(engine-host): real-bd-gated parameterized lifecycle + contention`.
...
- **(response-gemini-1-issue-1)** The `RouteBroadcaster` is grounded in the **durable event log** (`readSpineRuntimeRouteEvents`); command-returned deltas are **low-latency hints** that trigger an immediate broadcaster read, eliminating polling delay — they are never the broadcast payload, so hints and the poll tick can't double-publish (ID-diff dedupes).

## Open decisions (explicitly unsettled — for the executing session)

- Exact bundled-catalog install entrypoint name (`installQuestCatalog` vs an `init.ts` helper) — confirm against `spine-cli/src/commands/init.ts`.
