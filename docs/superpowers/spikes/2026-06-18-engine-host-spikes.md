...
**Date:** 2026-06-18 · **Gate for:** slice-1 build (Tasks 1-15) · **Beads:** runner-al4
...
- Exposing Spine authoring/runtime ops as Pi tools = Pi **extensions** or an rpc tool-bridge (slice 2).
...
The seam is present across every folder-host entry point, under **four field names** (all satisfied by one `SpineBeadsCliIssueClient`):
- `startQuestRoute` → `beadsClient` (`SpineBeadsIssueClient`)
- read-model (`listSpineRuntimeRoutes`/`getSpineRuntimeRoute`/`listSpineRuntimeTasks`) → `beadsReader` (`SpineBeadsIssueSnapshotReader`)
- transitions (`approveRouteGate`/`rejectRouteGate`/`pauseSpineRoute`/`resumeSpineRoute`/`resolveSpineRouteBlocker`, via `SpineBeadsTransitionOptions`) → `beadsMutator` (`SpineBeadsIssueMutationClient`)
- events + discussion read-model → `beadsReader` + `beadsCommentReader`

**Task 5 impact:** WorkspaceSession constructs **one** `SpineBeadsCliIssueClient` per workspace and threads it through every call under the matching field name. **No folder-host modification required** (better than the plan's contingency of adding the seam). Confirm `SpineBeadsCliIssueClient` implements all four interfaces (it is the default fallback for each, so it does).
