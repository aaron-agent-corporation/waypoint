...
**Date:** 2026-06-19 · **Issue:** waypoint-r66 · **Status:** both spikes resolved; build gate cleared.
...
(`docs/superpowers/plans/2026-06-18-waypoint-pi-agent-brain.md`). Throwaway scratch was under
`/tmp/pi-spike`; the durable outputs are the captured fixtures in
`packages/waypoint-engine-host/src/brain/__fixtures__/pi-stream/` and this doc.
...
end: a `waypoint_ping` tool read `process.env.WAYPOINT_HOST_URL` and the agent called it, returning
...
- `tool_execution_end` → `'agent.tool_result'` (carry `toolName`, `result.content[].text`, `isError`) — **this is where the adapter extracts `proposalId`/`adhocRouteId`** from the Waypoint tool's returned content.
...
`runWaypointAutopilot` (`packages/waypoint-folder-host/src/autopilot/run.ts`) resolves a recipe via
`loadRecipeManifest(projectRoot, slug)` which walks **one hardcoded directory**:
`join(getWaypointProjectPaths(projectRoot).waypointDir, 'recipes')` (`run.ts:393-399`). The route is
materialized from the local quest manifest (`.waypoint/quests/<slug>.yaml`) — `startQuestRoute`'s
...
1. Add `catalogDir?: string` to `RunWaypointAutopilotOptions`; thread it into `loadRecipeManifest`
   (default `<waypointDir>/recipes`, overlay `<waypointDir>/agent/<sessionId>/catalog/recipes`).
2. New `startAdhocRoute(projectRoot, { sessionId, questYaml, recipeYamls, dryRun?, signal? })`
   ...
   `.waypoint/agent/<sessionId>/catalog/{quests,recipes}/`, materialize the route + tasks from the
   ...
   call `runWaypointAutopilot(projectRoot, { routeId, catalogDir: overlayRecipesParent, signal })`.
   No writes to the live `.waypoint/quests|recipes` → no collision/shadowing (Lock 3).
...
3. `runWaypointAutopilot({ signal })` passes `signal` into each `runRecipe` call and stops the loop
   when aborted.
