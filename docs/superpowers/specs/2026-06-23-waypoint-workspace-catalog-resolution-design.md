...
catalog under the project's `.waypoint/` directory:
...
package catalog** (`loadBundledWaypointCatalog()`). The net effect is a dead-end:
...
- Autopilot's default recipe lookup (no ad-hoc `catalogDir`) reads recipes from a
  ...
  was never copied into `.waypoint/recipes` would **start but fail at execution**.
...
and store it in route metadata (`routeMetadata` → `waypoint.workflow`). The real
...
3. **New loader returns the existing `BundledWaypointCatalog` interface** (D3),
   making it a drop-in replacement at every call site.
4. **`loadBundledWaypointCatalog` is left unchanged** (D4) and is composed by the
...
5. **Autopilot default recipe lookup is in scope, bounded to recipe-manifest
   ...
   reference a bundled recipe that was never copied into `.waypoint/recipes`. The
...
6. **CLI catalog commands resolve via upward `.waypoint/` discovery** (D6).
   `quests.ts` and `recipes.ts` locate the project by searching upward for the
   nearest `.waypoint/` ancestor (not `process.cwd()`), so invocation from a
   subdirectory still sees approved workspace entries; when no `.waypoint/`
   ancestor exists they degrade to bundled-only. `init` and catalog-installation
   flows stay on `loadBundledWaypointCatalog()`.
...
2. loads workspace entries from `<projectRoot>/.waypoint/quests` +
   `<projectRoot>/.waypoint/recipes` (tolerant of either dir being absent);
...
`loadBundledWaypointCatalog` is **unchanged** (D4) — it remains the bundled
...
`createWaypointCatalog`); the workspace `root` for entry loading is the project's
...
- **no `catalogDir`** (the default path) → resolve through
  `loadWorkspaceWaypointCatalog(projectRoot)`, i.e. **workspace > bundled**.
...
`loadWorkspaceWaypointCatalog(projectRoot)` (or, for autopilot, route the default
...
| CLI `quests` | `waypoint-cli/src/commands/quests.ts` | direct CLI listing sees workspace-approved quests (upward `.waypoint/` discovery) |
| CLI `recipes` | `waypoint-cli/src/commands/recipes.ts` | direct CLI listing sees workspace-approved recipes (upward `.waypoint/` discovery) |
| autopilot default recipe lookup | `waypoint-folder-host/src/autopilot/run.ts` | default (non-`catalogDir`) `loadRecipeManifest` resolves authored + bundled recipes |
...
- The CLI commands (`quests.ts`, `recipes.ts`) find `projectRoot` by walking up
  from the invocation directory to the nearest `.waypoint/` ancestor; if none is
  found they fall back to bundled-only (no throw). `init` / catalog-install flows
  remain on `loadBundledWaypointCatalog()`.
- `autopilot/run.ts` routes **only** the default `loadRecipeManifest` lookup (no
  ad-hoc `catalogDir`) through `loadWorkspaceWaypointCatalog(projectRoot)`; the
...
- **CLI invoked outside any `.waypoint/` workspace**: no `.waypoint/` ancestor
...
- empty-but-present `.waypoint/quests` dir → bundled-only, no throw;
...
- `waypoint quests` / `waypoint recipes` include workspace-approved manifests when
  run from a temp initialized project, **including from a subdirectory** of that
  project (exercises upward `.waypoint/` discovery, D6);
- running the same commands outside any workspace still lists bundled entries.

**Autopilot regression** (folder-host):

- a route task referencing a bundled recipe **not** present in `.waypoint/recipes`
...
- `packages/waypoint-folder-host/src/catalog/bundled.ts` — export internal
...
- `packages/waypoint-folder-host/src/autopilot/run.ts` — route the default
  (non-`catalogDir`) recipe lookup through the workspace loader (D5).
- `packages/waypoint-cli/src/commands/quests.ts` — upward `.waypoint/` discovery,
  use the workspace loader (D6).
- `packages/waypoint-cli/src/commands/recipes.ts` — upward `.waypoint/` discovery,
  use the workspace loader (D6).
...
- **D1–D4** — overlay/workspace-wins; all resolution consumers; drop-in
  `BundledWaypointCatalog`; bundled loader unchanged.
...
- **D6** — CLI `quests`/`recipes` use upward `.waypoint/` discovery, not
  `process.cwd()`; bundled-only fallback when no workspace root.
- Symbol for reuse is `createCatalogRegistry` (`createWaypointCatalog` was a
...
- Re-confirm `runWaypointAutopilot` does not read the `workflow` field as a file.
