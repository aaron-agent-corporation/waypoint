# Waypoint Pi Agent Brain (Slice 2) — Design Spec
...
**Builds on:** slice 1 (`@waypoint/engine-host`, shipped: commit `f01a2c7`). Slice 3 = Tauri shell + UI (later).
...
2. **Tool exposure:** a **Pi extension** registers Waypoint operations as first-class Pi
...
5. **Run model:** **ad-hoc, UNRESTRICTED.** The agent runs its authored draft as an
   **ephemeral route** (no catalog promotion needed) — pulling ad-hoc execution (the
   `waypoint-j3b` gap) into slice 2 — and is **not** constrained by recipe side-effect policy
...
**Purpose.** Turn natural-language intent into an authored Waypoint workflow, **run it
...
- A **Pi extension** registering Waypoint tools that call the engine-host HTTP API with the
...
- An **ad-hoc execution path**: instantiate + run an authored draft as an **ephemeral route**
  without static-catalog promotion (resolves `waypoint-j3b` for the agent). **Unrestricted.**
...
- **`PiCliBrainAdapter`** — spawns `pi -p --mode json` with `--no-skills`/scoped `--tools`,
  the Waypoint extension (`-e`), a scoped system prompt, and env carrying the engine-host
...
- **Waypoint Pi extension** — registers the granted tools; each tool calls the engine-host
...
New package code under `packages/waypoint-engine-host/src/`:
```
brain/
...
`packages/waypoint-pi-extension/`) that registers the Waypoint tools against the host HTTP API.
...
4. Transcript persistence location/format for `agent.author` (`.waypoint/agent/<sessionId>...`).
