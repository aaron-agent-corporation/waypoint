# Spine UI — Recipe-Detail Surface Design
...
`metadata.runner.recipe.slug`, while the Beads backend writes it flat at
`metadata.runner.recipe_slug` (`beads/compiler.ts:291`). The UI must read both.
...
(`spine-ui/vite.config.ts`), so the browser client can call it today with no
...
// the slug nested at metadata.runner.recipe.slug; the Beads backend writes it
// flat at metadata.runner.recipe_slug (beads/compiler.ts:291). Both are read
// here so every consumer — selectTask, the badge, the graph subtitle — agrees.
function recipeSlugOf(task: Task): string | null {
  const wp = (task.metadata as { runner?: { recipe?: { slug?: string }; recipe_slug?: string } } | undefined)?.runner
...
confirmed in `packages/spine-folder-host/src/tasks/types.ts`),
...
- **`recipeSlugOf` helper:** resolves the **nested** folder-host path
  (`metadata.runner.recipe.slug`) *and* the **flat** Beads path
  (`metadata.runner.recipe_slug`); returns null for an empty-string slug, a
  missing `runner` block, and a task whose metadata is a JSON string rather
...
4. **Payload-risk wording.** Quantified with the concrete spec example
   ...
   figure.
5. **Task-kind count.** `SpineFolderTaskKind` is a **12-value** union; "not a
...
- **G1 — backend-specific recipe-slug path (verified against source).** The
  three-vendor run resolved the slug at `metadata.runner.recipe.slug` only. The
  Beads backend writes the slug flat at `metadata.runner.recipe_slug`
...
- **Slug-vs-`node.type` consistency.** The integrated draft's risk section said
  recipe-ness could be inferred from "an explicit `runner.node.type` override,"
