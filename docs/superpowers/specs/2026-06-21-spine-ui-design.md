# Spine UI (Slice 3, MVP) — Design

**Status:** Approved design (slice 3 of 3 — MVP scope)

**Builds on:** slice 1 (`@projectrunner/spine-engine-host`, shipped) + slice 2 (Pi agent
...
The larger Spine desktop app decomposes into: (1) engine host + local API
...
New workspace package `packages/spine-ui/` (Vite + React + TS). Depends on
`@projectrunner/spine-engine-host` for **types only** (envelope + event record types) and
on `@xyflow/react`.
...
packages/spine-ui/
  ...
  src/
    ...
    engine/
      ...
      types.ts            # re-export EngineEnvelope / AgentEventRecord / SpineFolderRoute|Task types
...
1. Reads the handshake file at the path given by `SPINE_ENGINE_HANDSHAKE`
   ...
   if `SPINE_ENGINE_HANDSHAKE` is unset, the plugin logs setup guidance (set
