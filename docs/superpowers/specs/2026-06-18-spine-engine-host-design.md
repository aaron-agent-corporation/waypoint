# Spine Engine Host + Local API — Design Spec
...
Spine is a host-agnostic TypeScript runtime — a "spine" for lifecycle +
workflow execution — currently exposed as `@projectrunner/spine` (portable engine),
`packages/spine-cli` (operator surface), and `examples/host-minimal` (a
reference host). It is deliberately a library/plugin: **no resident service, no
LLM-driven brain, and no UI.** Backends are folder / Beads-Dolt / Crew.

The larger goal is a **Spine desktop app** with:
...
1. **Engine host + local API** *(this spec)* — the resident sidecar that wraps
   `@projectrunner/spine` and exposes the full run/watch/author contract with live
   state streaming. Foundation the other two consume.
2. **Pi agent brain** — Pi SDK loop in the sidecar, Spine authoring +
...
**Purpose.** Create a resident process (`@projectrunner/spine-engine-host`) that
instantiates a Spine host (folder or Beads backend) and exposes the full
...
- One long-running process that owns a Spine workspace and its backend
  lifecycle.
...
- Reuse `@projectrunner/spine` contracts + `@projectrunner/spine-folder-host`; no host-specific
...
- No new backend type beyond what core already provides. Folder and Beads/Dolt
  are both in scope and first-class; Crew supervision is out of scope here.
...
│                                                                            │
│   Transport adapters          Command/Event core           Spine host  │
│  ┌──────────────────┐        ┌────────────────────┐      ┌──────────────┐ │
│  │ HTTP+WS (slice 1)│ <────> │  CommandBus        │ ───> │ @runner/   │ │
...
- **Command/Event core** is pure: a `CommandBus` (named commands → `{ok,...}`
  ...
  that fans out `SpineEventRecord`s from the host's `IEventBus`. It knows
  nothing about HTTP.
...
- **Workspace session** binds to one active workspace (folder root / Beads
  ...
  process.
- Lives as a new workspace package: `packages/spine-engine-host` (sibling to
  `spine-cli`, `spine-folder-host`).
...
- `run.start` `{ quest, backend? }` — start a quest (≈ `runner start`)
- `routes.list` / `route.get` `{ routeId }` / `route.events`
  `{ routeId, limit?, offset? }`
- `tasks.list` `{ routeId? }`
- `run.resume` `{ routeId }` / `run.pause` `{ routeId }`
- `gate.decide` `{ routeId, gateId, decision, note? }` (≈ `runner gate`)
...
- The host's `IEventBus` is the single source of live truth. `EventHub`
  subscribes once and re-broadcasts `SpineEventRecord`s to all connected
  clients.
...
New package `packages/spine-engine-host/`:
...
- **Parity**: assert engine-host commands produce envelopes consistent with the
  ...
  `docs/runner-envelope-parity-matrix.md`).

---

## Acceptance criteria

- [ ] `packages/spine-engine-host` exists as a workspace package depending on
      `@projectrunner/spine` + `@projectrunner/spine-folder-host`, passing `pnpm typecheck`.
