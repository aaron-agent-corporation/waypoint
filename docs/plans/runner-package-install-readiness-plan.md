# Spine Package + Install Readiness Implementation Plan

> **For Hermes:** Use subagent-driven-development skill to implement this plan task-by-task.

**Goal:** Make the standalone Spine repo consumable as installable built packages, with a built CLI that can be packed, installed into a temp project, and used for the folder-host journey.
...
- Root package: `@projectrunner/spine`, `private: true`, current exports point at `./src/index.ts`.
- Internal packages: `@projectrunner/spine-folder-host` and `@projectrunner/spine-cli`, both `private: true`, current exports/bin point at `src/*.ts`.
...
- Do not globally install `runner` on Aaron's machine as the primary proof; use temp-project install smokes.
...
- Create: `docs/plans/runner-package-install-readiness-plan.md`
...
- Possibly create: `packages/spine-cli/src/bin-built-shim.ts` only if the existing CLI entrypoint cannot compile cleanly as the bin.
...
- `dist/src/index.js`
- `dist/packages/spine-folder-host/src/index.js`
- `dist/packages/spine-cli/src/index.js`
- `dist/packages/spine-cli/src/bin.js`
...
- Root `@projectrunner/spine`: `main`, `types`, and `exports` point to `dist/src/index.js` / `.d.ts`.
- `@projectrunner/spine-folder-host`: `main`, `types`, and `exports` point to built files relative to that package or to a package-local dist strategy chosen in B1.
- `@projectrunner/spine-cli`: `bin.runner` points at built `bin.js` and exports point at built `index.js`.
...
node dist/packages/spine-cli/src/bin.js --help
pnpm typecheck
```
...
```bash
git add package.json packages/spine-folder-host/package.json packages/spine-cli/package.json tsconfig.build.json tsconfig.json scripts/verify-built-packages.mjs src/__tests__/package-build.test.ts
git commit -m "feat(package): add build pipeline for runner packages"
```
...
3. Built CLI import exposes `runSpineCli`.
...
git commit -m "test(package): verify built runner package boundaries"
```
...
4. Run the installed `runner` bin, not the source-run Node path:
...
runner tasks --route-id route-001
```

5. Assert `.runner/config.yaml`, route YAML, events JSONL, and task YAML exist in the temp project.
6. Delete the temp project unless `SPINE_KEEP_PACKAGE_SMOKE_PROJECT=1`.
...
```bash
git add package.json scripts/package-install-smoke.mjs src/__tests__/package-install-docs.test.ts docs/spine-folder-host.md
git commit -m "test(package): add install smoke for runner cli"
```
...
**Objective:** Document exactly how Mission Control should consume the standalone Spine package next.

**Files:**
- Create: `docs/plans/runner-package-consumption.md`
- Modify: `docs/plans/runner-remaining-roadmap.md`
- Modify: `docs/spine-folder-host.md` if operator install guidance changes.
...
```bash
pnpm exec vitest run src/__tests__/folder-host-docs.test.ts src/__tests__/runner-docs.test.ts
pnpm typecheck
```
...
```bash
git add docs/plans/runner-package-consumption.md docs/plans/runner-remaining-roadmap.md docs/spine-folder-host.md
git commit -m "docs(package): define runner consumption strategy"
```
...
**Files:**
- Create: `docs/plans/runner-package-readiness-closeout.md`
- Modify: `docs/plans/runner-remaining-roadmap.md`
...
```bash
git tag runner-package-rc-YYYYMMDD
```

Then push only after verifying local tag and user-approved remote target if needed:

```bash
git show --no-patch --oneline runner-package-rc-YYYYMMDD
...
```bash
git add docs/plans/runner-package-readiness-closeout.md docs/plans/runner-remaining-roadmap.md
...
- `pnpm smoke:package-install` installs the package into a temp project and runs the Spine folder-host journey from the installed `runner` bin.
