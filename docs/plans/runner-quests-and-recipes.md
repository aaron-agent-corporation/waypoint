...
This document defines the two core first-class concepts in Spine going forward:

1. **Quest** — a named, reusable workflow template. A Quest defines the full journey a project takes (e.g., discuss → plan → execute → verify → ship). Quests are the unit a project picks when it adopts Spine.
...
Quests compose Recipes. Projects adopt Quests. The runtime (`@projectrunner/spine`) executes the DAG a Quest defines, dispatching to Recipes at each node.
...
The `gsd_*` prefix is retired in all new naming. Tables and types that used `gsd_*` are renamed to `runner_*` (tracked in the remaining-roadmap doc).

## Folder layout

A Spine installation (whether MC-hosted or folder-hosted) exposes:

```
runner/
├── quests/
│   ├── runner.yaml                    # the GSD lifecycle as a Quest
...
│
├── recipes/
│   ├── doc-writer.yaml             # ported from runner-doc-writer
│   ├── planner.yaml                # ported from runner-planner
│   ├── executor.yaml               # ported from runner-executor
│   ├── verifier.yaml               # ported from runner-verifier
│   ├── debugger.yaml               # ported from runner-debugger
│   └── ... (one per ported agent)
│
└── templates/                      # optional: per-Quest project templates
    └── gsd/
        └── .runner/              # pre-populated state files
```

In folder-host mode, this layout lives under `.runner/` at the project root. In MC-hosted mode, it lives in the MC repo (`workflows/`, plus a new `recipes/` directory).
...
prompt: |
  You are a document writer for the Spine Spine Quest. Your job is to draft
  ...
  [... full prompt body, ported from runner-doc-writer.md ...]
...
This:
1. Reads `quests/runner.yaml`
2. Scaffolds the lifecycle skeleton in the project (workstream → milestone → phases) per the Quest's `scaffolds:` block
3. Writes project-local state to `.runner/` (folder-host) or DB rows (MC-host)
4. Sets the project's default Quest to `runner`
...
Projects can also adopt multiple Quests (e.g., a long-running `runner` Quest for main development plus ad-hoc `bugfix` Quests that run on-demand).

## Core contract additions

To formalize this, `@projectrunner/spine` gets:

- `packages/runner-core/src/quests/quest-manifest.ts`
...
The existing `workflows/runner-plan-execution.yaml` etc. stay valid — they're just workflow definitions, unchanged. What's new is:
...
3. **DB/folder rename `gsd_*` → `runner_*`** (tracked in the roadmap doc; mechanical rename, not conceptual change).

## Out of scope for this doc

- Exact port of GSD's 33 agents → that's in `runner-runner-quest-port.md`.
- Folder-host CLI wiring → that's in `spine-folder-host.md`.
...
1. `QuestManifest` and `RecipeManifest` types + parsers exist in `@projectrunner/spine`.
2. A `quests/runner.yaml` and at least one `recipes/*.yaml` exist and parse cleanly.
...
4. The `runner-lifecycle-workflow` port plan (`runner-runner-quest-port.md`) is authored and can begin execution.
