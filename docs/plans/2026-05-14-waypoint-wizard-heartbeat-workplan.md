...
- [x] WW6.3 — Add `waypoint wizard plan --case <case-root> --json`
  - Completed: added CLI `wizard plan` generation from existing `.waypoint/shadows`, persisted one-question-at-a-time ambiguity questions, wrote `.waypoint/wizard/adoption-plan.yaml` (or safe `--write-plan` override), and returned JSON/human summaries; `pnpm exec vitest run src/wizard/__tests__/plan.test.ts src/wizard/__tests__/questions.test.ts packages/waypoint-cli/src/commands/wizard.test.ts` passed with 14 tests on 2026-05-14. Full `pnpm typecheck` still has unrelated existing failures in `src/__tests__/firmvault-quest-skeleton.test.ts` and `src/operators/__tests__/instructions.test.ts`.
...
- [x] WW8.3 — Update docs and paralegal skill with Wizard usage
  - Completed: added `docs/waypoint-wizard.md`, updated folder-host command/docs coverage, and patched the paralegal FirmVault Waypoint skill with the Wizard command rail and safety boundaries; `pnpm exec vitest run src/__tests__/waypoint-docs.test.ts packages/waypoint-cli/src/commands/wizard.test.ts` and `pnpm smoke:waypoint-wizard-firmvault` passed on 2026-05-14.
