...
**Goal:** Cleanly separate Waypoint as the product/runtime name from bundled Quest names, then add a gstack-derived project-management Quest as a formal folder-settable starter Quest.

**Architecture:** Waypoint remains the system name. Bundled starter Quests carry independent method names: `firmvault`, `project-delivery`, and a new gstack-derived Quest such as `product-sprint`. The first gstack integration ports methodology and role prompts into Quest/Recipe manifests; it does not embed or execute gstack's Claude Code/browser/runtime machinery.
...
- Waypoint current starter Quest manifest: `quests/waypoint.yaml`
- FirmVault starter metadata: `quests/firmvault.yaml`
- Catalog listing command: `packages/waypoint-cli/src/commands/quests.ts`
- Catalog test: `packages/waypoint-cli/src/commands/catalog.test.ts`
- gstack source files:
  - `README.md`
  - `package.json`
  - `LICENSE`
...
**Waypoint is the product/system/runtime name. It must not be the public name of any Quest.**

Correct public model:

- **Waypoint** — the product/runtime/repo.
...
Do not present “Waypoint” as a Quest name. In prose, say “a Waypoint Quest named Project Delivery,” not “the Waypoint Quest.”
...
That maps naturally to a Waypoint Quest scaffold. The useful portable unit is the methodology and specialist-role prompts, not gstack's host-specific runtime.
...
### Task 1: Rename the current GSD-derived starter Quest away from `waypoint`
...
- Modify: `docs/quests/waypoint.md` → `docs/quests/project-delivery.md`
- Modify: `README.md`
- Modify: `docs/waypoint-folder-host.md`
- Modify: `docs/waypoint-quest-catalog.md`
- Modify: `docs/waypoint-core-integration.md`
- Modify tests that assert `waypoint` Quest slug/name:
  - `packages/waypoint-cli/src/commands/catalog.test.ts`
  - folder-host tests currently installing `quest: 'waypoint'`
...
1. Add/modify a catalog test asserting primary starters include:
   ...
   - no primary starter with slug `waypoint`
2. Run:

```bash
pnpm exec vitest run packages/waypoint-cli/src/commands/catalog.test.ts
```

Expected RED: test fails because current catalog still prints `- waypoint: Project Delivery (GSD)`.

3. Rename manifest slug and docs references.
4. Run targeted tests again.
5. Run broader catalog/folder-host tests.

**Compatibility note:** If too many internal tests or user folders assume `--quest waypoint`, do this in two commits:

- Commit A: public name becomes `Project Delivery`; metadata marks `waypoint` as deprecated alias.
- Commit B: add first-class alias resolution if needed, then migrate slug to `project-delivery`.

Do not leave public docs saying the `waypoint` Quest is the starter.
...
- Rewrite host-specific commands into Waypoint Recipe guidance.
...
- No Quest named `Waypoint` appears in the starter list.

```bash
pnpm exec vitest run src/__tests__/product-sprint-quest.test.ts packages/waypoint-cli/src/commands/catalog.test.ts
```
...
- Add test under `packages/waypoint-folder-host/src/routes/start.test.ts` or a new `src/__tests__/product-sprint-folder-smoke.test.ts`.
...
tmp=$(mktemp -d)
cd "$tmp"
node /Users/aaronwhaley/Github/active\ projects/waypoint/packages/waypoint-cli/src/bin.ts init --quest product-sprint
node /Users/aaronwhaley/Github/active\ projects/waypoint/packages/waypoint-cli/src/bin.ts start --quest product-sprint
node /Users/aaronwhaley/Github/active\ projects/waypoint/packages/waypoint-cli/src/bin.ts tasks --route-id route-001
```

**Assertions:**

- `.waypoint/quests/product-sprint.yaml` exists.
- `.waypoint/tasks/tasks.yaml` exists.
- tasks include Product Sprint phases.
- no repo-root `.waypoint/` contamination.
...
```text
Set up a Waypoint Quest for this folder:
...
```text
Waypoint Quest — Project Delivery
```
...
```bash
pnpm exec vitest run src/__tests__/product-sprint-quest.test.ts src/__tests__/product-sprint-recipe-port.test.ts packages/waypoint-cli/src/commands/catalog.test.ts
pnpm test
pnpm typecheck
pnpm build
...
- `waypoint quests` shows primary starters with independent Quest names:
  - `firmvault: FirmVault`
  - `project-delivery: Project Delivery`
  - `product-sprint: Product Sprint`
- No public docs call `waypoint` a Quest name.
