...
Port the source CLI (`get-shit-done-cc` / `get-shit-done-main`) into Waypoint as:

1. **One Quest** — `quests/waypoint.yaml` — the full discuss → plan → execute → verify → ship lifecycle as a Waypoint Quest.
...
- Author `quests/waypoint.yaml` as a Quest that mirrors GSD's lifecycle: initialize → discuss → plan → execute → verify → ship.
...
All prefixes drop the `waypoint-` — Recipes are namespaced under `recipes/` and the Quest context makes ownership clear.

### Commands to map (60+)
From `/Users/aaronwhaley/Downloads/get-shit-done-main/commands/gsd/`. Categorized by target:

**→ Quest phase entrypoints** (these become core phases in `quests/waypoint.yaml`)
...
### Shape of the main Quest (`quests/waypoint.yaml`) — high level
...
Source: `/Users/aaronwhaley/Downloads/get-shit-done-main/agents/waypoint-doc-writer.md`
...
version: 1
description: |
  Drafts project documents. Ported from GSD waypoint-doc-writer agent.
source:
  project: get-shit-done-cc
  file: agents/waypoint-doc-writer.md
...
prompt: |
  [full body of waypoint-doc-writer.md, verbatim or near-verbatim,
...
The port is **content preservation first, adaptation second**. Goal: if an operator loved GSD's doc-writer, they get the same behavior when running the Waypoint Quest in Waypoint.
...
- **P1.1** Write a small port script: read each `agents/waypoint-*.md`, extract prompt body, produce `recipes/<slug>.yaml` skeleton.
...
- **P2.2** Draft `quests/waypoint.yaml` using the high-level shape above, adjusted by what the commands actually do.
- **P2.3** Add Quest parse test: `quests/waypoint.yaml` parses cleanly, every `recipe:` reference resolves.
...
For each source command in the "operator actions" category, document the Waypoint equivalent in `docs/quests/waypoint.md`. No code changes — this is an operator-facing doc that says "in GSD you ran `/gsd pause-work`; in Waypoint you run `/waypoint pause --route-id N`."

### Phase P6 — Integration smoke test
- **P6.1** In the folder-host repo (see `waypoint-folder-host.md`), add a test fixture that runs `waypoint init --quest waypoint` and verifies the scaffolded `.waypoint/` layout.
- **P6.2** Add a dry-run test that materializes a route from `quests/waypoint.yaml` and validates the DAG can be traversed (nodes ordered correctly, gates present, recipe refs resolve, no cycles).
...
### Phase P7 — Doc pass
- **P7.1** Write `docs/quests/waypoint.md` — operator guide: what the Quest does, when to use it, how it compares to the source CLI it was ported from.
...
- **P8.2** Commit a closure note: "Waypoint Quest port complete (33 recipes, N Quests, N sub-Quests)."
...
2. `quests/waypoint.yaml` exists, parses cleanly, and every recipe reference resolves.
...
5. Operator-facing doc `docs/quests/waypoint.md` exists and explains how to adopt the Quest.
6. The Waypoint folder-host can run `waypoint init --quest waypoint` and produce a valid `.waypoint/` layout (tested via fixture).
...
4. **Drift over time.** GSD may evolve. Our port is a snapshot. Mitigation: record `ported_at` in each Recipe; document in `docs/quests/waypoint.md` that this is a point-in-time port, not a live mirror.
...
- **Soft dependency:** `waypoint-folder-host.md` — the folder-host CLI is what makes `waypoint init --quest waypoint` real. Without it, the Quest can still be parsed and validated, but not instantiated. (Parse/validate is enough for this plan to be "done.")
