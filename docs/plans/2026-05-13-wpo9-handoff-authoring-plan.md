...
Add Waypoint authoring support for draft handoff graph manifests so users can brainstorm/design handoffs and then generate safe, non-installed handoff YAML drafts through the same authoring workflow used for recipes and quests.
...
- `pnpm exec vitest run src/authoring/__tests__/handoff-generator.test.ts src/authoring/__tests__/questionnaires.test.ts packages/waypoint-cli/src/commands/author.test.ts`
- `pnpm build`
