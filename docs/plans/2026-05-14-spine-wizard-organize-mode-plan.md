# Spine Wizard Organize Mode Implementation Plan

> **For Hermes:** Use subagent-driven-development skill to implement this plan task-by-task.

**Goal:** Add a user-requested Spine Wizard mode that turns a messy source folder into a clean, organized Spine case workspace while preserving source-file read-only safety and legal-state boundaries.

**Architecture:** Keep the current shadow/adoption flow as the safe default, then add an explicit organize mode that can create a canonical Spine-facing case package: clean document tree, markdown indexes/summaries, source manifest, missing-document checklist, Wizard questions, and an adoption plan. Source files remain read-only unless the user explicitly opts into copying files into a new Spine-owned target folder. FirmVault legal facts are still changed only through approved safe state APIs.

**Tech Stack:** TypeScript, Node fs/path/crypto APIs, YAML frontmatter, existing `src/wizard/*` modules, `packages/spine-cli`, `packages/spine-folder-host`, Vitest, direct Node CLI smokes.
...
> "Here are messy user files. Create Spine shadows and map them safely."

Organize mode answers:

> "Here are messy user files. Fix them into a clean Spine system I can actually use."
...
2. **Explicit copy mode:** if real files are copied, they are copied into a new Spine-owned target, never moved in place.
...
6. **Apply remains safe:** approved legal-state changes call existing FirmVault safe state APIs; never hand-edit `.runner/firmvault/*.yaml` or landmarks.
7. **No external side effects:** no emails, faxes, filings, payments, calls, API actions, or trust actions.

## Desired user journey

```bash
runner wizard organize \
  --source /path/to/messy-files \
  --target /path/to/new-runner-case \
  --domain firmvault \
  --copy-files \
  --json

runner wizard questions --case /path/to/new-runner-case --json
runner wizard answer --case /path/to/new-runner-case --question <id> --answer <text> --json
runner wizard plan --case /path/to/new-runner-case --json
runner wizard apply --case /path/to/new-runner-case --plan .runner/wizard/adoption-plan.yaml --json
...
- Creates markdown shadows under `.runner/shadows`.
...
**Objective:** Convert scan + shadow classification output into a deterministic `.runner/wizard/organization-plan.yaml`.
...
**Objective:** Write the clean Spine/FirmVault folder layout plus useful markdown navigation files.
...
- `.runner/wizard/source-manifest.yaml`
- `.runner/wizard/organization-plan.yaml`
- `.runner/wizard/organize-report.md`
- `.runner/wizard/missing-documents-checklist.md`
...
### Task O3.1: Add `runner wizard organize`
...
- Modify: `docs/spine-wizard.md`

**CLI:**

```bash
runner wizard organize --source <path> --target <case-root> --domain <domain> [--copy-files] [--json]
```
...
**Files:**
- Create: `scripts/spine-wizard-organize-smoke.mjs`
- Modify: `package.json`
- Modify: `docs/spine-wizard.md`
...
```bash
pnpm smoke:spine-wizard-organize
```

**Smoke must prove:**
1. Temp messy corpus is created.
2. `runner wizard organize --copy-files` creates a clean target case.
...
5. `runner wizard plan` works on the organized case.
...
```bash
pnpm exec vitest run src/wizard/__tests__ packages/spine-cli/src/commands/wizard.test.ts
pnpm smoke:spine-wizard-organize
...
- `runner wizard organize` documented in CLI help and docs.
