# Spine Beads Runtime Contract

**Status:** Runtime parity implemented for the folder host, CLI, and Hermes/FirmVault adapter surfaces
**Date:** 2026-05-27
**Beads:** `runner-cdn`, `runner-beads-ops`

## Goal

Use Beads as a durable graph backend for Spine route work without changing
Spine's core product boundary.

Spine remains the portable runtime language: Quests, Recipes, Routes,
...
This is a backend contract, not a rebrand. Spine should not become Beads,
Mission Control, FirmVault, or GSD. Beads should not absorb Spine's catalog
or policy semantics.

For the optional agent/session runtime layer on top of this Beads backend, see
[`2026-05-27-runner-crew-runtime.md`](./2026-05-27-runner-crew-runtime.md).
Crew is treated there as a host adapter/supervisor target, not as a new
...
```bash
runner init --quest runner --backend beads
runner init --quest runner --backend beads --init-beads
runner firmvault bootstrap --cases-root /trusted/FirmVault/Cases --case-name "Jane Smith v. Acme Trucking" --case-type personal-injury --backend beads --init-beads --start --json
```

The Beads backend is now wired through the normal operator surfaces:

- `runner start` materializes the route graph as Beads issues and dependencies while preserving a compatibility route record.
...
- `runner init --backend beads --init-beads` provides an explicit real-workspace setup path using `bd init --non-interactive --skip-agents --skip-hooks`.
- `runner status --json` exposes `beads.readiness` and Beads root issue ids for operator/Hermes health checks.
- `runner start` checks Beads readiness before graph writes and fails with an action hint when `bd` or `.beads/` is missing.
...
The remaining `.runner/` files in Beads mode are local host configuration,
...
|---|---|---|
| `@projectrunner/spine` | Quest/Recipe manifest parsing, route concepts, subject scope, host contracts, safety semantics | Beads CLI calls, Dolt state, filesystem layout, host-specific execution |
| Spine host adapter | Backend selection, Beads command/API bridge, metadata encoding, status reconstruction | Catalog semantics, bypassing gates, recipe policy decisions |
...
`packages/spine-folder-host` while the integration is proven.

## Concept Mapping

| Spine concept | Beads representation | Notes |
|---|---|---|
...
| Workstream/milestone/phase/plan scaffold | Child issue hierarchy or metadata groups | Preserve scaffold refs so status can reconstruct Spine progress. |
...
| Recipe node | Task issue with `runner.recipe_slug` metadata | Beads tracks readiness and status; `IRecipeRuntime` executes. |
...
| Route event | Beads note/comment plus optional Spine event record | Beads is durable; adapters may keep JSONL events for compatibility during migration. |

## Metadata Contract

Every Beads issue materialized by Spine should include a stable metadata
...
not change or reinterpret the `runner` namespace without a version bump.

## Dependency Semantics

Beads `blocks` dependencies are the runtime readiness primitive. The compiler
must translate Spine ordering into blockers:
...
| Beads state | Spine route/task meaning |
|---|---|
...
Spine status reconstruction should be derived from Beads issue state and
...
`folder` preserves the existing `.runner/routes`, `.runner/tasks`, and
...
or external PR status. The canonical Spine route gate should initially be a
durable Beads issue with Spine metadata so route status survives sync,
...
4. **Status reconstruction.** Read the Beads graph back into Spine route and
   task summaries.
...
- Reconstruct a Spine route summary from Beads issue state.
...
- Whether the Beads bridge should live permanently in
  `packages/spine-folder-host` or graduate into a separate
  `packages/runner-beads-host` package.
- Whether formula export should target Beads formulas directly or first use a
  Spine-owned intermediate JSON graph.
...
- Beads-backed route events are reconstructed from route metadata and
  Spine-tagged Beads comments. Folder-backed routes continue to use JSONL
  route events.
- Beads-backed task discussion is stored as Spine-tagged comments on the
...
- FirmVault bootstrap selects the route backend through the same host-level
  backend selector as `runner init`.
