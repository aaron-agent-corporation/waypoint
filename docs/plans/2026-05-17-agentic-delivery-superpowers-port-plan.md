...
**Goal:** Add an attributed Superpowers-derived Agentic Delivery starter Quest to Spine, backed by source-derived Recipes and catalog/docs coverage.

**Architecture:** Treat Superpowers as source methodology/provenance, not Spine branding. Port each Superpowers skill into a namespaced Recipe (`agentic-delivery-*`) and compose them into one end-to-end Quest (`agentic-delivery`) for AI-assisted software delivery. Keep route nodes safe by default: planning/review/gate/checkpoint behavior only; no external publishing, merge, PR, or filesystem branch side effects without later explicit host wiring.

**Tech Stack:** Spine Quest/Recipe YAML manifests, existing recursive Quest/Recipe loaders, Vitest catalog/docs tests, TypeScript CLI catalog surface.
...
- Spine remains the runtime/system name only.
...
- Recipe slugs are namespaced as `agentic-delivery-<skill-slug>` to avoid collisions with existing Spine skills or Recipes.
...
- Modify: `packages/spine-cli/src/commands/catalog.test.ts`
...
1. Add a test that loads bundled Quests/Recipes and expects:
   ...
   - `metadata.runner.selection_summary === 'disciplined AI-assisted software delivery from idea through verified branch finish'`.
...
```bash
pnpm exec vitest run src/__tests__/agentic-delivery-quest.test.ts packages/spine-cli/src/commands/catalog.test.ts src/__tests__/folder-host-docs.test.ts
```
...
2. Preserve the source skill body in `prompt: |` with a short Spine wrapper header only if needed.
3. Record metadata:
...
**Objective:** Surface Agentic Delivery as a starter Quest without renaming Spine or Superpowers.

**Files:**

- Modify: `docs/spine-folder-host.md`
- Modify as needed: `docs/runner-quest-catalog.md`
- Modify as needed: `docs/plans/runner-source-port-status.md`
- Modify as needed: `SPINE_RESUME_PLAN.md`
...
4. Do not present Superpowers as Spine-owned branding.
...
```bash
pnpm exec vitest run src/__tests__/agentic-delivery-quest.test.ts packages/spine-cli/src/commands/catalog.test.ts src/__tests__/folder-host-docs.test.ts src/__tests__/runner-docs.test.ts
pnpm typecheck
pnpm test
pnpm build
...
```bash
git add docs/plans/2026-05-17-agentic-delivery-superpowers-port-plan.md src/__tests__/agentic-delivery-quest.test.ts packages/spine-cli/src/commands/catalog.test.ts src/__tests__/folder-host-docs.test.ts docs/spine-folder-host.md docs/runner-quest-catalog.md docs/plans/runner-source-port-status.md SPINE_RESUME_PLAN.md quests/agentic-delivery.yaml recipes/agentic-delivery
