...
**Goal:** Add a BMAD-derived software-delivery methodology as a formal folder-settable Spine Quest without using BMAD as a Spine product name or embedding the BMAD runtime.

**Architecture:** Spine remains the product/runtime. The new Quest is presented publicly as **Agile Delivery** with slug `agile-delivery`; BMAD/BMad Method is preserved only as source attribution and compatibility/provenance metadata. The first implementation ports BMAD's methodology catalog into Spine Quest/Recipe manifests and starter-menu docs; it does not execute BMAD's installer, module runtime, or external side-effect workflows.
...
- Spine catalog files inspected:
  - `packages/spine-cli/src/commands/quests.ts`
...
**Do not call the public Spine Quest “BMAD,” “BMad,” or “BMad Method.”**
...
> I can set up this folder with a Spine Quest. Current starter options are FirmVault, Project Delivery, Product Sprint, and Agile Delivery. Agile Delivery is best when you want a structured software lifecycle: analysis, PRD, UX, architecture, epics/stories, sprint execution, QA, and retrospective.

---

## Assessment: is BMAD a good Quest candidate?

Yes. BMAD maps extremely cleanly to Spine's Quest/Recipe model because it already contains:
...
BMAD should be adapted as a **Spine Quest + Recipe catalog**, not as a runtime dependency.
...
- Any BMAD brand/logo assets into Spine UI/docs except plain-text attribution links.
...
metadata:
  ...
  source:
    ...
    license: MIT
    trademark_notice: BMAD/BMad/BMad Method are trademarks of BMad Code, LLC; Spine uses this only as attribution/provenance.
scaffolds:
  workstreams:
    - key: software
      name: Agile Delivery
...
prompt: |
  ...
  Preserve the source workflow intent, but enforce Spine safety boundaries:
...
- Modify test: `packages/spine-folder-host/src/catalog/bundled.test.ts`
...
- `metadata.runner.quest_family === 'primary_starter'`;
...
```bash
pnpm exec vitest run src/__tests__/agile-delivery-quest.test.ts packages/spine-folder-host/src/catalog/bundled.test.ts
```
...
```bash
git add quests/agile-delivery.yaml workflows/agile-delivery.yaml recipes/agile-delivery src/__tests__/agile-delivery-quest.test.ts packages/spine-folder-host/src/catalog/bundled.test.ts
...
```bash
pnpm exec vitest run src/__tests__/runner-docs.test.ts src/__tests__/folder-host-docs.test.ts
```
...
**Objective:** Ensure `runner quests` lists Agile Delivery in primary starter Quests.

**Files:**

- Modify: `packages/spine-cli/src/commands/quests.test.ts` or existing CLI catalog test file.
- Modify only if required: `packages/spine-cli/src/commands/quests.ts`
...
```bash
pnpm exec vitest run packages/spine-cli/src/commands/quests.test.ts
```
...
```bash
pnpm exec vitest run src/__tests__/agile-delivery-quest.test.ts packages/spine-folder-host/src/catalog/bundled.test.ts packages/spine-cli/src/commands/quests.test.ts
pnpm test
pnpm typecheck
pnpm build
...
- Spine must not execute BMAD installer/runtime commands as part of the Quest.
- The Quest can create plans, prompts, checklists, tasks, and review artifacts.
- External effects such as publishing, deploying, sending, or filing require explicit human approval and a Spine gate.
- Preserve source attribution and MIT license notices.
- Do not use BMAD trademarks as Spine product/Quest branding.
