# Spine Package Distribution Hardening Plan

> **For Hermes:** Use test-driven-development skill to implement this plan task-by-task.

**Goal:** Make Spine packages publish-ready for Mission Control consumption by exact pinned package versions, with an automated preflight that catches private-package flags, leaked `workspace:*` dependencies, and clean-consumer import failures.

**Architecture:** Keep Spine as the portable runtime. Add a release/prepublish verification script that exercises the actual packed artifacts for `@projectrunner/spine`, `@projectrunner/spine-folder-host`, and `@projectrunner/spine-cli` in a clean temp project without local workspace overrides. Update package metadata only as needed so private registry publishing is possible while preserving the existing monorepo workspace development flow.

**Tech Stack:** Node.js ESM scripts, pnpm pack/install, package.json metadata, TypeScript build output, existing Spine package entrypoints.

---

## Context

Mission Control P0.2 found that local tarball install only worked with a consumer-side `pnpm.overrides` workaround for `@projectrunner/spine`. That is acceptable for spike/debug but not for merge-ready Mission Control integration.

The next Spine-side requirement is to make the package artifacts publishable and self-consistent so Mission Control can consume exact private-registry versions such as:

```json
{
  "dependencies": {
    "@projectrunner/spine": "0.1.0-mc.0",
    "@projectrunner/spine-folder-host": "0.1.0-mc.0"
  }
}
```
...
1. Run `pnpm build`.
2. Pack `@projectrunner/spine`, `@projectrunner/spine-folder-host`, and `@projectrunner/spine-cli` into a temp directory.
...
7. Use local-only `pnpm.overrides` inside that temp project to simulate private-registry resolution of exact transitive `@runner/*` dependencies; do not use this override shape in Mission Control.
8. Run `pnpm install --ignore-scripts`.
9. Import and assert core/folder-host/cli exports.
10. Run `pnpm exec runner --version`.
...
**Files:**
- Modify: `docs/plans/runner-package-consumption-strategy.md`
