# Spine Package Consumption Strategy

**Status:** Track 5 / B4 decision record  
**Date:** 2026-05-07  
**Purpose:** Define how downstream consumers, especially Mission Control, should consume Spine after the build/install readiness work.

---

## Ground truth

Spine currently ships from a private pnpm workspace:

- root package: `@projectrunner/spine`
- workspace package: `@projectrunner/spine-folder-host`
- workspace package / CLI bin: `@projectrunner/spine-cli` with `runner`
...
{
  "dependencies": {
    "@projectrunner/spine": "github:Whaleylaw/runner#<tag-or-sha>"
  }
}
```
...
- Spine is private and only Mission Control needs it.
...
- More than one system consumes Spine.
...
2. Publish `@projectrunner/spine`, `@projectrunner/spine-folder-host`, and `@projectrunner/spine-cli` in dependency order.
...
{
  "dependencies": {
    "@projectrunner/spine": "file:../runner"
  }
}
```
...
Mission Control cutover should not start until a Spine commit has passed:
...
- the exact Spine package tag/SHA/version
...
2. Tag the verified Spine commit, e.g. `runner-package-rc.1` or a semver pre-release tag.
