# Waypoint Wizard Implementation Plan

> **For Hermes:** Use subagent-driven-development skill to implement this plan task-by-task.

**Goal:** Build Waypoint Wizard: a generic folder-to-shadow adoption layer that scans arbitrary user files, creates canonical markdown shadows with source pointers and PII metadata, asks one-question-at-a-time clarifications, generates a durable adoption plan, and applies approved FirmVault facts through existing safe state APIs.

**Architecture:** Implement generic Wizard core first, then add the FirmVault adapter. The Wizard must treat source files as immutable evidence, create Waypoint-owned markdown shadows under `.waypoint/shadows`, persist Q&A/adoption artifacts under `.waypoint/wizard`, and use existing FirmVault state/guidance APIs for legal progress.

**Tech Stack:** TypeScript, `yaml`, Node fs/path/crypto APIs, Vitest, Waypoint CLI in `packages/waypoint-cli`, folder-host/FirmVault state APIs in `packages/waypoint-folder-host`, local smoke scripts.
...
- Paralegal skill context: `/Users/aaronwhaley/.hermes/profiles/paralegal/skills/case-management/firmvault-waypoint-case-operations/SKILL.md`

## Non-negotiable rules

1. **No source mutation by default.** User files are read-only inputs.
2. **Markdown shadows are the Waypoint working layer.** Waypoint organizes shadows, not the user's original tree.
3. **Shadows do not satisfy legal facts.** They only support proposed evidence mappings.
4. **Apply uses safe state APIs.** No raw `.waypoint/firmvault/*.yaml` edits for legal state.
...
2. Runs `waypoint wizard scan` without mutating source files.
3. Runs `waypoint wizard shadow` into a temp Waypoint case.
4. Confirms shadows exist under `.waypoint/shadows/firmvault/...` with source pointers and hashes.
5. Runs `waypoint wizard questions` and sees pending clarification.
6. Runs `waypoint wizard answer` for fixture clarifications.
7. Runs `waypoint wizard plan` and sees proposed FirmVault facts.
8. Marks a small subset of facts approved in the fixture plan.
9. Runs `waypoint wizard apply`.
10. Confirms state changed only through existing FirmVault APIs.
11. Runs `waypoint firmvault guidance --json` and confirms updated landmarks/next actions.
...
- shadow output paths must stay under `.waypoint/shadows/<domain>/`;
- wizard artifact paths must stay under `.waypoint/wizard/`;
...
## Task WW2.2: Add `waypoint wizard scan` CLI

**Objective:** Expose read-only scan through CLI.

**Files:**
- Create: `packages/waypoint-cli/src/commands/wizard.ts`
- Create/modify: `packages/waypoint-cli/src/commands/wizard.test.ts`
...
```ts
await runWaypointCli(['wizard', 'scan', '--source', tempSource, '--domain', 'firmvault', '--json'], io)
```
...
Wire `wizard scan` in `runWaypointCli` and top-level help.
...
```bash
git add packages/waypoint-cli/src/commands/wizard.ts packages/waypoint-cli/src/commands/wizard.test.ts packages/waypoint-cli/src/bin.ts
...
- shadows are written under `.waypoint/shadows/firmvault/...`;
...
## Task WW3.2: Add `waypoint wizard shadow` CLI
...
```bash
waypoint wizard shadow --source <temp-source> --target <temp-case> --domain firmvault --json
```
...
```bash
pnpm exec vitest run packages/waypoint-cli/src/commands/wizard.test.ts src/wizard/__tests__/shadows.test.ts
```
...
```bash
pnpm exec vitest run packages/waypoint-cli/src/commands/wizard.test.ts src/wizard/__tests__/questions.test.ts
```
...
```bash
git add packages/waypoint-cli/src/commands/wizard.ts packages/waypoint-cli/src/commands/wizard.test.ts src/wizard/questions.ts
...
**Objective:** Generate `.waypoint/wizard/adoption-plan.yaml` with source map, shadow records, proposed facts, questions/answers, warnings, and missing expected FirmVault docs.
...
## Task WW6.2: Add `waypoint wizard plan` CLI
...
```bash
waypoint wizard plan --case <case-root> --json
```
...
```bash
pnpm exec vitest run packages/waypoint-cli/src/commands/wizard.test.ts src/wizard/__tests__/plan.test.ts
```
...
```bash
git add packages/waypoint-cli/src/commands/wizard.ts packages/waypoint-cli/src/commands/wizard.test.ts src/wizard/plan.ts
...
```bash
pnpm exec vitest run src/wizard/__tests__/firmvault-apply.test.ts packages/waypoint-folder-host/src/firmvault/state.test.ts
```
...
## Task WW7.3: Add `waypoint wizard apply` CLI
...
```bash
pnpm exec vitest run packages/waypoint-cli/src/commands/wizard.test.ts src/wizard/__tests__/firmvault-apply.test.ts
```
...
**Files:**
- Create directory: `examples/waypoint-wizard/firmvault-messy-source/`
- Create: `examples/waypoint-wizard/firmvault-messy-source/Intake Docs.pdf.txt`
- Create: `examples/waypoint-wizard/firmvault-messy-source/random/DocuSign Complete.pdf.txt`
- Create: `examples/waypoint-wizard/firmvault-messy-source/medical/Smith Records.pdf.txt`
- Create: `examples/waypoint-wizard/firmvault-messy-source/unknown/scan001.txt`
- Create: `examples/waypoint-wizard/README.md`
...
```bash
git add examples/waypoint-wizard
...
**Files:**
- Create: `scripts/waypoint-wizard-firmvault-smoke.mjs`
- Modify: `package.json`
...
```json
"smoke:waypoint-wizard-firmvault": "node scripts/waypoint-wizard-firmvault-smoke.mjs"
```
...
```bash
git add scripts/waypoint-wizard-firmvault-smoke.mjs package.json
git commit -m "test(wizard): add FirmVault wizard smoke"
```

## Task WW8.3: Update docs and paralegal skill

**Objective:** Teach users and the paralegal profile how to use Waypoint Wizard.

**Files:**
- Create: `docs/waypoint-wizard.md`
- Modify: `docs/waypoint-folder-host.md` if command list is maintained there
- Modify: `/Users/aaronwhaley/.hermes/profiles/paralegal/skills/case-management/firmvault-waypoint-case-operations/SKILL.md`
...
```bash
node /Users/aaronwhaley/Github/waypoint/packages/waypoint-cli/src/bin.ts wizard scan --source <source> --domain firmvault --json
node /Users/aaronwhaley/Github/waypoint/packages/waypoint-cli/src/bin.ts wizard shadow --source <source> --target <case-root> --domain firmvault --json
node /Users/aaronwhaley/Github/waypoint/packages/waypoint-cli/src/bin.ts wizard questions --case <case-root> --json
node /Users/aaronwhaley/Github/waypoint/packages/waypoint-cli/src/bin.ts wizard answer --case <case-root> --question <id> --answer <text> --json
node /Users/aaronwhaley/Github/waypoint/packages/waypoint-cli/src/bin.ts wizard plan --case <case-root> --write-plan .waypoint/wizard/adoption-plan.yaml --json
node /Users/aaronwhaley/Github/waypoint/packages/waypoint-cli/src/bin.ts wizard apply --case <case-root> --plan .waypoint/wizard/adoption-plan.yaml --json
```

**Step 3: Verify docs smoke/tests**

```bash
pnpm exec vitest run src/__tests__/waypoint-docs.test.ts packages/waypoint-cli/src/commands/wizard.test.ts
pnpm smoke:waypoint-wizard-firmvault
```
...
```bash
git add docs/waypoint-wizard.md docs/waypoint-folder-host.md
git commit -m "docs(wizard): document Waypoint Wizard workflow"
```
...
```bash
pnpm exec vitest run src/wizard/__tests__/*.test.ts packages/waypoint-cli/src/commands/wizard.test.ts
pnpm smoke:waypoint-wizard-firmvault
pnpm build
...
```bash
git add <files>
git commit -m "chore(wizard): finalize Waypoint Wizard gates"
```
