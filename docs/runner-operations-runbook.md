...
Operational guide for running Waypoint safely in a host.

## Safety Defaults

- Discussion auto-response is **OFF by default**.
- Auto-response request emission requires:
  1. task metadata opt-in: `runner.discussion.auto_response.enabled: true`
...
- `/runner doctor [--definition runner-doctor] [--version 1]`
- `/runner forensics [--definition runner-forensics] [--version 1]`
- `/waypoint discuss --task-id <id> [--message <text>]`

## Typed API Surface

- `GET /api/projects/:id/runner/status`
- `POST /api/projects/:id/runner/command`
- `GET|POST /api/projects/:id/runner/routes`
- `GET /api/projects/:id/runner/routes/:routeId`
- `GET /api/projects/:id/runner/routes/:routeId/events`
- `POST /api/projects/:id/runner/routes/:routeId/state`
- `POST /api/projects/:id/runner/routes/:routeId/gate`
- `GET|POST /api/projects/:id/runner/autopilot`
...
- If needed, rollback to previous commit on `feat/runner-runtime-slice` after verifying migration-free diff.

## Verification Commands

```bash
pnpm exec vitest run \
  src/app/api/projects/[id]/runner/command/__tests__/route.test.ts \
  src/app/api/projects/[id]/runner/routes/__tests__/route.test.ts \
  src/app/api/projects/[id]/runner/routes/[routeId]/events/__tests__/route.test.ts \
  src/app/api/projects/[id]/runner/routes/[routeId]/gate/__tests__/route.test.ts \
  src/app/api/projects/[id]/runner/routes/[routeId]/state/__tests__/route.test.ts \
  src/app/api/projects/[id]/runner/autopilot/__tests__/route.test.ts \
  ...
  src/lib/__tests__/runner-task-discussion.test.ts
