...
host-specific types**. It is deliberately portable.
...
project follows (e.g. "Client Onboarding", "Project Delivery (GSD)", "bugfix", "research
...
`docs/plans/runner-quests-and-recipes.md` for the full design.
...
runtime:
  workflow: runner-plan-execution
  version: 1
...
const quest = questsResult.registry.get('runner')
...
See `examples/runner-host-minimal/`. It provides an executable,
...
```
pnpm exec vitest run examples/runner-host-minimal/src/
```
