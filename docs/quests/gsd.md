# GSD Quest Operator Guide

The GSD Quest is the batteries-included Waypoint Quest port of the get-shit-done-cc working loop. It preserves the useful shape of the old GSD flow while moving the implementation into Waypoint primitives.
...
The main Quest lives at `quests/gsd.yaml` and models the journey:
...
A Recipe is the reusable agent definition. Recipes live under `recipes/gsd/` and preserve ported agent prompt content and orchestration intent from the source GSD agent library. The Quest does not hard-code an agent runtime; a host decides how to invoke a Recipe through its `IRecipeRuntime` adapter.

## How the old GSD concepts map to Waypoint

- Old GSD lifecycle commands become the main `gsd` Quest phase metadata.
...
- Standalone old GSD commands become separate Quest manifests when they describe a reusable journey, for example `quests/debug.yaml`, `quests/spike.yaml`, `quests/code-review.yaml`, and `quests/pr-branch.yaml`.
...
The full command mapping is in `docs/quests/gsd-command-map.md`; the machine-readable source of that map is `docs/quests/gsd-command-map.yaml`.
...
- Agent-specialized prompts, ported into namespaced `gsd-*` Recipes.
...
- Discuss: the Quest can open a task-scoped discussion with `gsd-doc-writer` to clarify objective and acceptance criteria.
...
Current Quest manifests list Recipe slugs under `recipes:` and carry phase/source-command intent under `metadata.gsd_port`. Structural tests verify every listed Recipe resolves through the Recipe registry.

Representative phase Recipes:

- Initialize: `gsd-doc-writer`, `gsd-project-researcher`, `gsd-roadmapper`
- Discuss: `gsd-assumptions-analyzer`, `gsd-codebase-mapper`, `gsd-doc-writer`
- Plan: `gsd-phase-researcher`, `gsd-planner`, `gsd-plan-checker`
- Execute: `gsd-executor`
- Verify: `gsd-verifier`, `gsd-debugger`, `gsd-plan-checker`
- Ship: `gsd-doc-synthesizer`, `gsd-code-reviewer`, `gsd-executor`

## Operator workflow

A host that adopts this Quest should present it as a Waypoint journey, not as a legacy GSD command clone.

1. Load Quests from `quests/` and Recipes from `recipes/`.
2. Choose the `gsd` Quest.
...
- It does not implement a standalone GSD CLI.
...
As of P6, the GSD port has:

- 33 namespaced GSD Recipes under `recipes/gsd/`.
- 1 main GSD Quest under `quests/gsd.yaml`.
- 35 command-informed/catalog Quest manifests plus the existing example Quest.
- 65 upstream command mappings in `docs/quests/gsd-command-map.yaml`.
- Structural smoke coverage in `src/__tests__/gsd-port.test.ts`.
- Operator documentation coverage in `src/__tests__/gsd-docs.test.ts`.
