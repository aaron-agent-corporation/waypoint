...
Paralegal/Perry agents must invoke the Waypoint CLI rather than an ad hoc direct builder when Aaron asks `Create a referral package for <folder>`. The short command is enough; the Quest carries the detailed medical chronology, START_HERE, QC, and handoff requirements.

From the trusted folder-host project/case root:

```bash
waypoint quests
waypoint recipes --quest referral-package
waypoint init --quest referral-package
waypoint start --quest referral-package --json
waypoint routes --json
waypoint tasks --route-id route-001 --json
waypoint route-events --route-id route-001 --json
```

Minimum route evidence before reporting a package as Quest-governed:

- `.waypoint/` exists at the case/project root;
- `waypoint routes --json` shows the started referral-package route;
- `waypoint tasks --route-id route-001 --json` shows the referral-package task chain, including medical chronology tasks when medical records are present;
- `waypoint route-events --route-id route-001 --json` records the route/task progression;
- package QC records whether the package was produced by a real Waypoint Quest route.

Missing `.waypoint` route evidence means the output is not Quest-governed and must be treated as an agent/direct-builder failure, not as a passed referral-package Quest.

medical records present → chronology artifacts required before handoff:

- `03-medical/MEDICAL_CHRONOLOGY.md` or equivalent chronology markdown;
- `03-medical/medical-chronology-output/medical-chronology.html`;
- timeline PDF and master binder PDF when generated by the chronology workflow;
- one extracted visit PDF per chronology row;
- adversarial chronology QC report showing the chronology passed or blocking handoff.

Do not mark the referral package handoff-ready from files alone. Handoff readiness requires package QC plus the `attorney-handoff-gate` route state, and no external delivery happens without human approval.
