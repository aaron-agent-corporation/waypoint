...
The `runner` Quest is the batteries-included Project Delivery (GSD) Quest port of the get-shit-done-cc working loop. It preserves the useful shape of the old GSD flow while moving the implementation into Waypoint primitives.
...
The main Quest lives at `quests/runner.yaml` and models the journey:
...
A Recipe is the reusable agent definition. Recipes live under `recipes/runner/` and preserve ported agent prompt content and orchestration intent from the source GSD agent library. The Quest does not hard-code an agent runtime; a host decides how to invoke a Recipe through its `IRecipeRuntime` adapter.

## How the old GSD concepts map to Waypoint

- Source lifecycle commands become the main `runner` Quest phase metadata.
...
- Old operational commands become Waypoint operator actions where the runtime already has an equivalent command surface, for example `/waypoint pause`, `/waypoint resume`, `/waypoint auto`, `/waypoint status`, `/waypoint routes`, and `/runner help`.
- Optional namespace commands are deferred until Waypoint has first-class namespace/context semantics.

The full command mapping is in `docs/quests/runner-command-map.md`; the machine-readable source of that map is `docs/quests/runner-command-map.yaml`.
...
- Agent-specialized prompts, ported into namespaced `runner-*` Recipes.
...
- Discuss: the Quest can open a task-scoped discussion with `runner-doc-writer` to clarify objective and acceptance criteria.
...
- Initialize: `runner-doc-writer`, `runner-project-researcher`, `runner-roadmapper`
- Discuss: `runner-assumptions-analyzer`, `runner-codebase-mapper`, `runner-doc-writer`
- Plan: `runner-phase-researcher`, `runner-planner`, `runner-plan-checker`
- Execute: `runner-executor`
- Verify: `runner-verifier`, `runner-debugger`, `runner-plan-checker`
- Ship: `runner-doc-synthesizer`, `runner-code-reviewer`, `runner-executor`
...
- 33 namespaced Waypoint Recipes under `recipes/runner/`.
- 1 main Project Delivery (GSD) Quest under `quests/runner.yaml`.
- 35 command-informed/catalog Quest manifests plus the existing example Quest.
- 65 upstream command mappings in `docs/quests/runner-command-map.yaml`.
- Structural smoke coverage in `src/__tests__/runner-port.test.ts`.
- Operator documentation coverage in `src/__tests__/runner-docs.test.ts`.
