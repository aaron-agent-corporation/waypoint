...
> **Historical document.** This design dates from Waypoint's original embedding
> inside a specific host product (2026-05). References to that host's Workflow
> Engine, lifecycle entities, and domain workflows (including the legal-domain
> deployment mentioned below) describe the original host environment, which is
> not part of this open-source repository. Retained for design rationale; the
> current host contract is `docs/waypoint-folder-host.md`.

**Status:** Draft v0.1  
**Date:** 2026-05-01  
**Owner:** Waypoint
**Scope:** the host project lifecycle + Workflow Engine integration

---

## 1. Executive summary

**Waypoint** is the original host's lifecycle-and-workflow runtime for moving projects through structured objectives, executable routes, gated checkpoints, recipe dispatch, and autonomous progress loops.

Waypoint is not a clone of GSD. GSD is the historical inspiration and guide. Waypoint is host-native: it keeps the existing project lifecycle hierarchy as the semantic layer and uses the Workflow Engine as the execution substrate.
...
| Waypoint Workstream | `runner_workstreams` | A parallel lane of project work. |
| Waypoint Milestone | `runner_milestones` | A major objective or delivery checkpoint. |
| Waypoint Phase | `runner_phases` | A lifecycle segment within a milestone. |
| Waypoint Plan | `runner_plans` | A concrete unit of planned work inside a phase. |
...
| Waypoint Dispatch | workflow recipe node materialization | Creation of the host tasks from route nodes. |
...
Documented in `docs/agent-runner-guide.md` and implemented around `src/lib/runner-hierarchy.ts` plus migration `053_gsd_hierarchy_foundation`.
...
| `projects.runner_phase` | Legacy top-level lifecycle: `discuss -> plan -> execute -> verify -> done`. |
...
| Table | Current role |
|---|---|
| `runner_workstreams` | Project-scoped lanes; status `active`, `paused`, `complete`. |
| `runner_milestones` | Project/workstream objectives; status `planned`, `active`, `complete`, `archived`. |
| `runner_phases` | Milestone-scoped lifecycle phases; status `planned`, `active`, `complete`, `deferred`. |
| `runner_plans` | Phase-scoped work plans; status `todo`, `in_progress`, `review`, `done`, `failed`. |

Existing task linkage columns:

- `tasks.runner_workstream_id`
- `tasks.runner_milestone_id`
- `tasks.runner_phase_id`
- `tasks.runner_plan_id`
- legacy `tasks.runner_phase`
...
(definition -> instance -> node instances -> dependencies -> events)
        ...
        | materializes
        v
the host tasks / recipe agents / reviews / gates
        |
...
|---|---|---|
| `runner_project` | `projects.id` as string | Route runs against an entire project. |
| `runner_workstream` | `runner_workstreams.id` as string | Route runs against a workstream. |
| `runner_milestone` | `runner_milestones.id` as string | Route runs against a milestone. |
| `runner_phase` | `runner_phases.id` as string | Route runs against a phase. |
| `runner_plan` | `runner_plans.id` as string | Route runs against a plan. |
...
But new Waypoint definitions should prefer `runner_*`.
...
When a Workflow Engine recipe/review node materializes a the host task for a Waypoint route, the task should receive:

| Task field | Source |
|---|---|
| `project_id` | materialization input / route vars |
| `runner_workstream_id` | route vars or lifecycle parent lookup |
| `runner_milestone_id` | route vars or lifecycle parent lookup |
| `runner_phase_id` | route vars or lifecycle parent lookup |
| `runner_plan_id` | route vars if subject is a plan |
...
```text
runner:{subject_type}:{subject_id}:{definition_slug}:v{definition_version}
```

Examples:

```text
runner:runner_milestone:42:runner-milestone-planning:v1
runner:runner_plan:88:runner-plan-execution:v1
```
...
- the chat session is local to the host/Waypoint by default, rather than forcing the operator back to Telegram/Slack;
...
- discussion tasks should be valid Workflow Engine nodes for Waypoint routes, especially `runner-project-intake`, `runner-milestone-planning`, and review/gate clarification.
...
| Current `runner_plans.status` | Waypoint meaning | Workflow interaction |
|---|---|---|
...
```text
runner.project.{project_id}.enabled == true
runner.milestone.{milestone_id}.planned == true
runner.phase.{phase_id}.ready == true
runner.plan.{plan_id}.approved == true
runner.plan.{plan_id}.done == true
```
...
|---|---|---|
| `runner-project-intake` | `runner_project` | Turn a rough objective into lifecycle structure. |
| `runner-milestone-planning` | `runner_milestone` | Inspect context, draft milestone plan, gate approval. |
| `runner-phase-planning` | `runner_phase` | Break a phase into executable plans/tasks. |
| `runner-plan-execution` | `runner_plan` | Execute a plan using coder/debugger/reviewer recipe nodes. |
| `runner-phase-verification` | `runner_phase` | Verify phase acceptance and transition readiness. |
| `runner-milestone-verification` | `runner_milestone` | Verify milestone acceptance and close/advance. |
| `runner-doctor` | `runner_project` | Diagnose inconsistent lifecycle/workflow/task state. |
| `runner-forensics` | `runner_project` | Explain failure/blockage timeline from events/logs/artifacts. |

### 7.2 Example: plan execution route

```yaml
schema_version: 1
id: runner-plan-execution
name: Waypoint Plan Execution
version: 1
subject_type: runner_plan
...
This example intentionally uses existing `runner-*` recipe slugs as leaf workers until recipe names are renamed or replaced.
...
| `/runner enable` | Enable Waypoint lifecycle for a project. |
| `/runner intake` | Start/continue project-intake route. |
| `/runner plan` | Start planning route for a project/milestone/phase. |
| `/runner execute` | Start execution route for a plan. |
...
| `/runner doctor` | Start diagnostic route. |
| `/runner forensics` | Start forensics route for blocked/failed scope. |

Compatibility aliases can support `/gsd ...` during transition, but user-facing docs should prefer `/runner ...`.
...
```json
{
  "project": { "id": 42, "name": "Example", "runner_enabled": true },
  "lifecycle": {
    "workstreams": [],
    "milestones": [],
    "active_phase": null,
  ...
  "routes": [
    {
      ...
      "definition_slug": "runner-plan-execution",
      "subject_type": "runner_plan",
      "subject_id": "88",
      "status": "active",
      "nodes": []
...
- Ensure materialized tasks receive `runner_workstream_id`, `runner_milestone_id`, `runner_phase_id`, and `runner_plan_id`.
- Add tests around route-to-task metadata.

### Phase 3 — Add initial Waypoint route definitions

- Add `runner-*` workflow YAML files under `workflows/`.
- Reuse existing `runner-*` leaf recipes initially.
...
1. Should new public REST routes be `/api/runner/...` immediately, or should Waypoint be a UI/CLI alias over existing `/api/projects/:id/gsd/...` routes first?
...
3. Should route instances be one-per-lifecycle-entity or one-per-operation? Example: one `runner_plan` execution route per plan, or separate planning/execution/verification routes per plan.
...
5. Should `runner_*` condition satisfaction be explicit API calls, derived from lifecycle transitions, or both?
6. How much of the existing `runner-*` recipe naming should be renamed in the first pass?
...
1. Add `docs/runner-runtime-design.md` to the repo. *(This document.)*
2. Create `docs/superpowers/plans/YYYY-MM-DD-runner-runtime-integration.md` with a task-by-task implementation plan.
3. Add tests for a `runner` scope resolver that maps subject types to project/workstream/milestone/phase/plan IDs.
4. Implement a small `src/lib/runner.ts` helper module for:
...
6. Add first `workflows/runner-plan-execution.yaml` definition and registry validation test.
