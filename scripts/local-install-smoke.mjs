...
const tempRoot = await mkdtemp(join(tmpdir(), 'waypoint-install-smoke-'))
...
try {
  ...
  run('pnpm', ['exec', 'waypoint', 'init', '--quest', 'waypoint'], { cwd: projectDir })
  run('pnpm', ['exec', 'waypoint', 'start', '--quest', 'waypoint'], { cwd: projectDir })
  run('pnpm', ['exec', 'waypoint', 'routes'], { cwd: projectDir })
  run('pnpm', ['exec', 'waypoint', 'tasks', '--route-id', 'route-001'], { cwd: projectDir })
