...
export function parseWaypointCommand(rawCommand: string): WaypointParsedCommand {
  ...
  if (head === 'doctor') {
    ...
    const definitionSlug = defFlagIdx >= 0 ? tokens[defFlagIdx + 1] : 'runner-doctor'
  ...
  if (head === 'forensics') {
    const defFlagIdx = tokens.findIndex((t) => t === '--definition')
    const definitionSlug = defFlagIdx >= 0 ? tokens[defFlagIdx + 1] : 'runner-forensics'
    if (!definitionSlug) throw new Error('Invalid --definition value')
  ...
  if (head === 'start' || head === 'execute') {
    ...
    const defFlagIdx = tokens.findIndex((t) => t === '--definition')
    const definitionSlug = defFlagIdx >= 0 ? tokens[defFlagIdx + 1] : 'runner-plan-execution'
    if (!definitionSlug) throw new Error('Invalid --definition value')
