...
import { parseHandoffManifest, isHandoffManifest, type HandoffManifest, type HandoffStep, type HandoffManifestParseResult, type HandoffManifestParseError } from '../manifest.ts'
...
handoffs:
  - slug: agent-to-attorney-demand-review
    from: acme-agent
    to: attorney-review
...
`
  ...
  it('parses a valid handoff manifest', () => {
    ...
    expect(result.manifest.slug).toBe('acme-agent-handoffs')
    expect(result.manifest.name).toBe('Acme Agent Handoffs')
    expect(result.manifest.handoffs).toHaveLength(1)

    const step = result.manifest.handoffs[0]
    expect(step.slug).toBe('agent-to-attorney-demand-review')
    expect(step.from).toBe('acme-agent')
...
slug: acme-agent-handoffs
name: Acme Agent Handoffs
description: Handoff graph for Acme agent workflow
handoffs:
  - slug: agent-to-attorney
    ...
    trigger: demand_package_ready
metadata:
  source: acme-port-part-six
`
    const result = parseHandoffManifest(yaml)

    expect(result.ok).toBe(true)
    if (!result.ok) throw new Error('expected success')
    expect(result.manifest.description).toBe('Handoff graph for Acme agent workflow')
    expect(result.manifest.metadata?.source).toBe('acme-port-part-six')
  })
