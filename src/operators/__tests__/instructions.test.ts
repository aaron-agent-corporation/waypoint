...
import type { OperatorManifest } from '../manifest.ts'
import { resolveOperatorInstructions } from '../instructions.ts'

async function makeRepo(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'runner-operator-instructions-'))
...
describe('operator instruction resolver', () => {
  it('returns ordered layers for an operator manifest', async () => {
    ...
    expect(resolved.operator_slug).toBe('acme-agent')
  ...
  it('resolves skill layers against explicit skill roots', async () => {
    const repoRoot = await makeRepo()
    const skillRoot = await mkdtemp(join(tmpdir(), 'runner-skill-root-'))
    await mkdir(join(skillRoot, 'case-management', 'acme-runner-case-operations'), { recursive: true })
    await writeFile(join(skillRoot, 'case-management', 'acme-runner-case-operations', 'SKILL.md'), '# Skill\n')
    const operator = operatorWithLayers([
      { kind: 'skill', ref: 'case-management/acme-runner-case-operations', required: true },
    ])

    const resolved = await resolveOperatorInstructions(operator, { repoRoot, skillRoots: [skillRoot] })

    expect(resolved.layers[0]).toMatchObject({ kind: 'skill', ref: 'case-management/acme-runner-case-operations', exists: true })
    expect(resolved.errors).toEqual([])
  })
