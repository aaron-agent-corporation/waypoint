...
import type { WizardAmbiguity } from './types.ts'
import { assertWithinRoot, safeWizardArtifactPath } from './paths.ts'
import type { WizardAnswer, WizardDomain, WizardQuestion } from './types.ts'

export interface GenerateWizardQuestionsInput {
  domain: WizardDomain
  ambiguities: WizardAmbiguity[]
...
function promptForAmbiguity(ambiguity: WizardAmbiguity): string {
  ...
  if (ambiguity.kind === 'unknown_classification') {
    return `What category should this shadow use? Shadow: ${ambiguity.related_shadow_paths.join(', ')}`
  }
