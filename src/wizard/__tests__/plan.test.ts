...
import { approveWizardProposedFacts, buildWizardAdoptionPlan, writeWizardAdoptionPlan } from '../plan.ts'
import type {
  WizardAnswer,
...
function shadow(kind: string, filename: string): WizardShadowRecord {
  return {
    id: `shadow-${filename}`,
    domain: 'documents',
    shadow_path: `.waypoint/shadows/documents/${kind}/${filename}.md`,
    source: {
...
function proposedFact(overrides: Partial<WizardProposedFact> = {}): WizardProposedFact {
  return {
    ...
    fact: 'documents.agreement.signed',
    status: 'proposed',
    evidence_shadow: '.waypoint/shadows/documents/contracts/Fee Agreement.md',
...
function organizedDocument(overrides: Partial<WizardOrganizedDocumentEntry> = {}): WizardOrganizedDocumentEntry {
  return {
    ...
    shadow_path: '.waypoint/shadows/documents/contracts/Fee Agreement.md',
    classification: {
      kind: 'contracts',
      confidence: 'high',
...
describe('Wizard adoption plans', () => {
  it('builds a durable safe adoption plan from shadows, facts, questions, answers, and review items', () => {
    const questions: WizardQuestion[] = [
      {
        ...
        prompt: 'What category should this shadow use?',
        status: 'pending',
        domain: 'documents',
    ...
    const plan = buildWizardAdoptionPlan({
      ...
      questions,
      answers,
      missingExpectedDocuments: ['documents.authorizations.release'],
    ...
    expect(plan).toMatchObject({
      ...
      source_root: '/tmp/source',
      target_case_root: '/tmp/case',
      missing_expected_documents: ['documents.authorizations.release'],
      warnings: ['Review unknown classifications before apply.'],
      safety: {
  ...
  it('includes source inventory, shadow map, classifications, Q&A, approvals, and review summaries', () => {
    ...
    expect(plan.shadow_map).toEqual([
      ...
      expect.objectContaining({
        shadow_path: '.waypoint/shadows/documents/unknown/scan-001.md',
  ...
  it('marks only selected proposed facts approved without applying unselected facts', () => {
    const plan = buildWizardAdoptionPlan({
      ...
      sourceRoot: '/tmp/source',
      targetCaseRoot: '/tmp/case',
      shadows: [shadow('contracts', 'Fee Agreement'), shadow('authorizations', 'Release Form')],
      proposedFacts: [
        proposedFact({
          id: 'fact-agreement',
          fact: 'documents.agreement.signed',
        ...
        proposedFact({
          id: 'fact-release',
          fact: 'documents.authorizations.release',
          evidence_shadow: '.waypoint/shadows/documents/authorizations/Release Form.md',
          source_path: '/tmp/source/Release Form.pdf',
        }),
      ],
      questions: [],
      answers: [],
    ...
    const approved = approveWizardProposedFacts(plan, ['fact-agreement'])

    expect(plan.proposed_facts.map((fact) => fact.approved)).toEqual([false, false])
    expect(approved.proposed_facts).toEqual([
      expect.objectContaining({ id: 'fact-agreement', approved: true }),
      expect.objectContaining({ id: 'fact-release', approved: false }),
    ])
