...
import { createWizardShadows } from '../shadows.ts'
import { parseWizardShadowMarkdown } from '../shadow-frontmatter.ts'
...
describe('createWizardShadows', () => {
  it('writes deterministic markdown shadows from scanned source files', async () => {
    ...
    for (const shadow of result.shadows) {
      ...
      expect(parsed.frontmatter.domain).toBe('documents')
    ...
    for (const relPath of relativePaths) {
      expect(relPath).toContain('.waypoint/shadows/documents/')
    ...
    // unknown classification files go under other/
    ...
    expect(unknownShadow!.shadow_path).toContain('other/')
  })
  ...
  it('generates deterministic shadow paths using slugified category and filename', async () => {
    ...
    // category from classifier: documents/  filename slugified: fee-agreement
    expect(shadow.shadow_path).toMatch(/shadows\/documents\/documents\/fee-agreement\.md$/)
  })
