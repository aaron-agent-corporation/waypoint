...
function ambiguity(overrides: Partial<FirmVaultWizardAmbiguity>): FirmVaultWizardAmbiguity {
  return {
    id: 'ambiguity-001',
...
describe('Wizard clarification questions', () => {
  it('turns FirmVault ambiguities into stable one-at-a-time questions', () => {
      ...
      ambiguities: [
        ambiguity({
          ...
          related_shadow_paths: [
            ...
            '.waypoint/shadows/firmvault/contracts/retainer.md',
          ],
        }),
        ambiguity({
          id: 'ambiguity-002',
    ...
    expect(questions[0]?.prompt).toContain('Which shadow should support client.contracts.fee_agreement?')
    expect(questions[1]).toMatchObject({
      id: 'question-ambiguity-002',
      status: 'pending',
      related_shadow_paths: ['.waypoint/shadows/firmvault/unknown/scan-001.md'],
    })
    expect(questions[1]?.prompt).toContain('What FirmVault category should this shadow use?')
  ...
  it('uses recorded answers to suppress already-resolved questions', () => {
    const questions = generateWizardQuestions({
      ...
      ambiguities: [
        ...
        ambiguity({ id: 'ambiguity-002', related_shadow_paths: ['.waypoint/shadows/firmvault/unknown/scan-002.md'] }),
      ],
    })
    ...
    expect(generateWizardQuestions({ domain: 'firmvault', ambiguities: [ambiguity({ id: 'ambiguity-001' })], answers })).toEqual([])
  })
