...
describe('Waypoint Wizard path safety helpers', () => {
  ...
  it('builds shadow output paths under the Wizard shadow namespace', () => {
    expect(safeShadowRelativePath('documents', 'Medical Records', 'Dr Smith Records.pdf')).toBe(
      '.waypoint/shadows/documents/medical-records/dr-smith-records.md',
    )
  })
  ...
  it('rejects traversal, backslash escapes, and empty path segments', () => {
    expect(() => safeShadowRelativePath('documents', '../escape', 'file.pdf')).toThrow(/Unsafe Wizard path segment/)
    expect(() => safeShadowRelativePath('documents', 'intake', '..\\escape.pdf')).toThrow(
      /Unsafe Wizard path segment/,
    )
    expect(() => safeShadowRelativePath('documents', '', 'file.pdf')).toThrow(/Unsafe Wizard path segment/)
