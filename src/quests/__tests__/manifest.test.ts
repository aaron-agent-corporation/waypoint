...
`
    ...
    if (!result.ok) return
    expect(result.manifest.slug).toBe('runner')
...
describe('questAvailability (D5)', () => {
  const withAvailability = (value: unknown) => ({ runner: { quest_family: 'acme_workstream', availability: value } })

  it('treats an absent field as available — every Quest that predates D5', () => {
    expect(questAvailability(undefined)).toBe('available')
    expect(questAvailability({})).toBe('available')
    expect(questAvailability({ runner: { quest_family: 'primary_starter' } })).toBe('available')
    expect(questIsAvailable({ runner: {} })).toBe(true)
  })

  it('reads the declared values', () => {
    expect(questAvailability(withAvailability('available'))).toBe('available')
    expect(questAvailability(withAvailability('not_yet_available'))).toBe('not_yet_available')
    expect(questIsAvailable(withAvailability('not_yet_available'))).toBe(false)
  })

  it('falls back to the guarded value, never the reassuring one', () => {
    // A typo, a future value, a wrong type — none of these may read as "go".
    expect(questAvailability(withAvailability('not-yet-available'))).toBe('not_yet_available')
    expect(questAvailability(withAvailability('AVAILABLE'))).toBe('not_yet_available')
    expect(questAvailability(withAvailability(true))).toBe('not_yet_available')
    expect(questAvailability(withAvailability(null))).toBe('not_yet_available')
  })

  it('ignores metadata shapes that are not mappings', () => {
    expect(questAvailability(null)).toBe('available')
    expect(questAvailability([1, 2])).toBe('available')
    expect(questAvailability({ runner: 'primary_starter' })).toBe('available')
  })
})
