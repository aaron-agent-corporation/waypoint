...
describe('buildWaypointRouteKey', () => {
  it('builds a stable colon-joined route key with a v-prefixed version', () => {
    const key = buildWaypointRouteKey({
      ...
      subjectId: 42,
      definitionSlug: 'runner-plan-execution',
      definitionVersion: 1,
    })
    expect(key).toBe('runner:runner_plan:42:runner-plan-execution:v1')
  })
  ...
  it('accepts string subject ids and numeric versions uniformly', () => {
    const key = buildWaypointRouteKey({
      ...
      subjectId: '7',
      definitionSlug: 'runner-doctor',
      definitionVersion: 2,
    })
    expect(key).toBe('runner:runner_project:7:runner-doctor:v2')
  })
  ...
  it('does not double-prefix versions that already start with v', () => {
    const key = buildWaypointRouteKey({
      subjectType: WAYPOINT_SUBJECT_TYPES.project,
      subjectId: 1,
      definitionSlug: 'runner-forensics',
      definitionVersion: 'v3',
    })
    expect(key).toBe('runner:runner_project:1:runner-forensics:v3')
  })
})
...
describe('normalizeWaypointScope', () => {
  ...
  it('treats compat gsd_* subject types as valid runner subjects', () => {
  ...
  it('rejects unsupported subject types', () => {
    expect(() =>
      normalizeWaypointScope({
        subjectType: 'not-a-runner-subject',
        subjectId: 1,
      }),
...
describe('isWaypointSubjectType', () => {
  it('accepts both runner_* and compat gsd_* subject types', () => {
