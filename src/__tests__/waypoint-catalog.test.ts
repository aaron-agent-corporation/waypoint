...
describe('Waypoint Quest/Recipe port structural smoke coverage', () => {
  it('loads the full Waypoint source port corpus with unique slugs and resolved recipes', async () => {
    ...
    const gsdRecipeSlugs = recipeSlugs.filter((slug) => slug.startsWith('waypoint-')).sort()
    expect(gsdRecipeSlugs).toHaveLength(33)
    expect(gsdRecipeSlugs).toEqual([
      'waypoint-advisor-researcher',
      'waypoint-ai-researcher',
      'waypoint-assumptions-analyzer',
      'waypoint-code-fixer',
      'waypoint-code-reviewer',
      'waypoint-codebase-mapper',
      'waypoint-debug-session-manager',
      'waypoint-debugger',
      'waypoint-doc-classifier',
      'waypoint-doc-synthesizer',
      'waypoint-doc-verifier',
      'waypoint-doc-writer',
      'waypoint-domain-researcher',
      'waypoint-eval-auditor',
      'waypoint-eval-planner',
      'waypoint-executor',
      'waypoint-framework-selector',
      'waypoint-integration-checker',
      'waypoint-intel-updater',
      'waypoint-nyquist-auditor',
      'waypoint-pattern-mapper',
      'waypoint-phase-researcher',
      'waypoint-plan-checker',
      'waypoint-planner',
      'waypoint-project-researcher',
      'waypoint-research-synthesizer',
      'waypoint-roadmapper',
      'waypoint-security-auditor',
      'waypoint-ui-auditor',
      'waypoint-ui-checker',
      'waypoint-ui-researcher',
      'waypoint-user-profiler',
      'waypoint-verifier',
    ])
  ...
  it('keeps the main Waypoint Quest scaffold explicit about discussion and human gates', async () => {
    ...
    const waypoint = quests.registry.get('waypoint')
    expect(waypoint).toBeDefined()
    if (!waypoint) throw new Error('missing waypoint Quest')

    const phases = flattenPhases(waypoint)
    ...
    expect(phasesBySlug.get('discuss')?.plans?.some((plan) => plan.metadata?.waypoint?.discussion?.agent === 'waypoint-doc-writer')).toBe(true)
