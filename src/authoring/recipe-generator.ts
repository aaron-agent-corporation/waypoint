...
export function generateAuthoringRecipeDraft(input: GenerateAuthoringRecipeDraftInput): GenerateAuthoringRecipeDraftResult {
  ...
  for (const tool of input.tools ?? []) {
    if (!isSafeToolSlug(tool)) errors.push(`tool ${tool} must be a safe tool slug`)
  }
      ...
      ...(input.domain === 'firmvault'
        ? {
            firmvault: {
              ...
              evidence_rule: 'case-local evidence only; documents do not satisfy landmarks automatically',
            },
          }
        : {}),
    },
