...
import { createAuthoringDraft, validateAuthoringDraft } from '../draft.ts'
...
describe('Waypoint authoring draft contract', () => {
  it('creates draft-only generated files with the brainstorming gate preserved', () => {
    const draft = createAuthoringDraft({
      ...
      brainstorm_context: { inspected_paths: ['quests/acme.yaml', 'recipes/acme'] },
      questions: [
      ...
      generated_files: [
        {
          path: 'docs/plans/generated-acme-followup-design.md',
    ...
    expect(draft.brainstorm_context.inspected_paths).toContain('quests/acme.yaml')
  ...
  it('rejects generated files that would install by default', () => {
    const draft = createAuthoringDraft({
      ...
      source: 'questionnaire',
      answers: {},
      brainstorm_context: { inspected_paths: ['operators/acme/agent.yaml'] },
      questions: [{ id: 'goal', prompt: 'Goal?', order: 1 }],
      approaches: [
