...
import { getAuthoringQuestionnaire } from '../questionnaires.ts'

describe('Waypoint authoring questionnaires', () => {
  it('keeps brainstorming first and lifecycle intent before execution tasks', () => {
    const questionnaire = getAuthoringQuestionnaire({ kind: 'quest', domain: 'acme' })

    expect(questionnaire.kind).toBe('quest')
    expect(questionnaire.domain).toBe('acme')
  ...
  it('adds handoff graph questions when brainstorming handoff manifests', () => {
    const questionnaire = getAuthoringQuestionnaire({ kind: 'handoff_graph', domain: 'acme' })
