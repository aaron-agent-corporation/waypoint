...
/** Shares the four-outcome run vocabulary (VERDICT G3) with the Crew
 * runtime; a local subprocess has no poll budget, so it never reports
 * `exhausted`. */
export type LocalRecipeRuntimeOutputStatus = 'finished' | 'failed' | 'stopped'
...
export class LocalRecipeRuntime {
  ...
  async runRecipe(input: LocalRecipeRuntimeInput): Promise<LocalRecipeRuntimeOutput> {
    ...
    return {
      status: result.aborted ? 'stopped' : result.exitCode === 0 ? 'finished' : 'failed',
      runtime: 'local',
      recipe: input.recipe,
