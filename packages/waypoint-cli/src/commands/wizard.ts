...
export async function runWizardOrganize(args: readonly string[], io: WaypointCliIo): Promise<number> {
  ...
  try {
    ...
    const response = {
      ...
      domain_facts_from_organization: plan.domain_facts_from_organization,
...
export async function runWizardPlan(args: readonly string[], io: WaypointCliIo): Promise<number> {
  ...
  try {
    ...
    const proposedFacts: WizardAdoptionPlan['proposed_facts'] = []
    const ambiguities: WizardAmbiguity[] = []
    ...
    const missingExpectedDocuments: WizardAdoptionPlan['missing_expected_documents'] = []
    ...
    if (jsonOutput) {
      ...
      io.stdout(`Wizard Adoption Plan`)
      io.stdout(`==============================`)
...
async function readWizardShadowRecords(caseRoot: string): Promise<WizardShadowRecord[]> {
  ...
  for (const markdownPath of markdownPaths) {
    ...
    records.push({
      ...
      shadow_path: parsed.frontmatter.runner.canonical_path,
...
function inferWizardPlanDomain(shadows: WizardShadowRecord[], organizationPlan?: WizardOrganizationPlan | null): WizardDomain {
  const [first] = shadows
  return organizationPlan?.domain ?? first?.domain ?? 'documents'
}
