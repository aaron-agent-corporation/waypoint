...
describe('waypoint tools command', () => {
  it('lists safe tools for an operator as JSON', async () => {
    ...
    expect(await runWaypointCli(['tools', 'list', '--operator', 'research-analyst', '--json'], io)).toBe(0)
    ...
    expect(parsed.tools).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ slug: 'example.search', side_effect_class: 'read_only' }),
        expect.objectContaining({ slug: 'example.summarize', side_effect_class: 'local_state_mutation' }),
      ]),
    )
  })
  ...
  it('explains a safe tool as JSON', async () => {
    ...
    expect(await runWaypointCli(['tools', 'explain', 'example.search', '--json'], io)).toBe(0)

    expect(stderr).toEqual([])
    const parsed = JSON.parse(stdout.join('\n')) as { slug: string; inputs: Array<{ name: string; required: boolean }> }
    expect(parsed.slug).toBe('example.search')
    expect(parsed.inputs).toEqual(expect.arrayContaining([expect.objectContaining({ name: 'query', required: true })]))
  })
