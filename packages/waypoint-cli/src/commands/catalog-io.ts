 ...
 * a clean stderr line + `null` return, never an unhandled rejection (runner-bae #4).
 */
export async function loadCliCatalog(io: WaypointCliIo): Promise<BundledWaypointCatalog | null> {
  ...
  try {
    ...
    // its stack so it isn't masked as routine (runner-sga F4). Summary first,
    // then the stack detail.
    io.stderr(`Failed to load the quest catalog: ${err instanceof Error ? err.message : String(err)}`)
